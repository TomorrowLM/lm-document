...
1. **数字（例如7B, 32B）**：表示模型的参数规模：
   - **7B**：表示模型有70亿个参数（B表示"billion"，即十亿）。
   - **32B**：表示模型有320亿个参数。
   - **0.5B**：表示模型有5亿个参数。
2. **字母（例如Instruct, VL, Coder）**：
...
Prompt 是人为构造的输入内容，用来引导大模型理解任务并生成符合预期的输出。Prompt Engineering 则是设计、优化提示词的过程，目标是提升 AI 输出的准确性、稳定性和可控性。

#### CRISPE / ISPE 框架

CRISPE 是常见的提示词组织框架，可以理解为：**先定角色，后说背景，再提要求，最后定风格**。

- **CR：Capacity and Role（能力与角色）**：指定 AI 扮演的角色，例如“前端架构师”“代码审查员”“产品经理”。
- **I：Insight（洞察）**：提供背景信息、上下文、限制条件和已有材料。
- **S：Statement（陈述）**：明确希望 AI 完成的任务、输出格式和验收标准。
- **P：Personality（个性）**：指定回答风格、专业身份、语气和处理方式。
- **E：Experiment（实验）**：要求 AI 给出多个方案、对比优缺点或进行迭代优化。

简化使用时，可以只保留 **ISPE**：背景 → 任务 → 风格 → 多方案。

#### 好 Prompt 的基本要素

- **目标明确**：不要只说“帮我优化”，而要说明优化什么、达到什么效果。
- **上下文充分**：提供相关代码、业务背景、接口字段、限制条件和已有结论。
- **指令具体**：明确要分析、修改、生成、对比还是验证。
- **格式清晰**：指定输出为 Markdown、表格、JSON、代码块或步骤清单。
- **示例驱动**：给出期望输入/输出示例，让模型更容易对齐结果。
- **分步执行**：复杂任务拆成“分析 → 方案 → 实现 → 验证”的链路。
- **约束边界**：说明不能做什么，例如“不改业务逻辑”“不新增依赖”“只修改当前文件”。
- **迭代反馈**：根据 AI 输出继续补充限制、纠正偏差、要求重写或收敛方案。

#### 常用提示技术

| 技术 | 含义 | 适用场景 |
| ---- | ---- | -------- |
| Zero-shot | 不给示例，直接让模型完成任务 | 简单问答、翻译、总结 |
| Few-shot | 提供少量示例后再让模型生成 | 格式固定、风格统一的任务 |
| Chain of Thought | 引导模型分步骤推理 | 复杂分析、数学、排障 |
| ReAct | 思考与行动交替推进 | Agent 调工具、多步骤执行 |
| Reflexion | 先生成，再反思修正 | 代码审查、方案优化 |
| Prompt Chaining | 多个 Prompt 串联完成复杂任务 | 长流程、文档生成、功能开发 |

#### 结构化输出

结构化输出能减少歧义，方便后续处理，常见格式包括：

- **Markdown**：适合文档、总结、方案说明。
- **表格**：适合对比、清单、验收项。
- **JSON**：适合程序读取、配置生成、接口参数描述。
- **代码块**：适合生成代码、命令、配置文件。
...
```text
你是一名前端架构师。
背景：当前项目使用 React + Ant Design，需要新增用户列表页。
任务：请先分析页面结构，再给出组件拆分和接口设计。
要求：不要写代码，只输出技术方案。
输出格式：使用 Markdown，包含“页面结构 / 组件拆分 / 数据流 / 风险点”。
```

### 上下文管理

- **任务模块化（Modularization）**：将大任务拆成可独立完成的小任务，逐步推进。
- **精准信息投喂（Targeted Feeding）**：只提供当前任务相关的函数、数据模型、接口或文档，避免无关上下文干扰。
- **及时切分会话**：如果任务之间没有上下文依赖，开启新对话，减少历史信息噪声。
- **阶段性固化结果**：完成一个子任务后，及时提交代码或沉淀结论，避免重复推理。

### Agent 使用策略

- **并行 Sub-agent**：当多个功能可以独立推进时，启动轻量级 Agent 分别处理，避免单个会话上下文过大。
- **关闭无关能力**：在 Cline 或类似 Agent 系统中，关闭当前不需要的 Skill、MCP 工具或插件。
- **规则约束**：通过技能、规则或项目规范限制 AI 的行为边界，提升输出一致性。

### 缓存与模型选择

- **不要在会话中途切换模型**：上下文缓存通常与模型绑定，中途切换模型会导致缓存重建，反而降低效率。
...
| 步骤 | 动作 | 说明 |
| ---- | ---- | ---- |
| Step 1 | 创建任务 | 根据 `spec/*.md` 为每个模块创建独立任务 |
...
- 6 个 Phase 按序执行，每阶段均有门控确认。
- Phase 3 选择按技术层拆分，生成 `01`、`02a`、`02b`、`03` 四类规格。
- Phase 4 使用 MCP 编排，按“创建任务 → 分批执行 → 等待完成 → 汇总结果 → 补齐结果”完成执行闭环。
- 规格验收结果：17/17 全部通过。
...
| 改进 | 说明 |
| ---- | ---- |
| `pages.yaml` 替代 `pages.json` | 注释支持更好，YAML 格式更适合页面索引 |
| pages.yaml schema 独立 | 主文件精简，细节按需加载 |
| `children` 仅用于路由嵌套 | 列表页不放 children，schema 约束更清晰 |
| spec 命名 `NNx-` 格式 | `01`、`02a`、`02b`、`03`，并行批次用字母后缀 |
| MCP 结果补齐 | 子任务缺失结果时，通过汇总和补齐步骤闭环 |
...
| 维度     | Tool（工具）                                               | MCP                                                          |
| :------- | :--------------------------------------------------------- | :----------------------------------------------------------- |
| 本质     | Agent 可调用的**单个能力单元**（如读文件、搜索、执行命令） | 一种**协议/标准**，用来接入外部工具服务                      |
| 类比     | 一个具体的"函数"                                           | 提供这些函数的"插件市场/第三方服务"                          |
| 关系     | 每个 Tool 都有名称+参数 schema                             | MCP Server 内可暴露**多个** Tool                             |
| 调用方式 | 直接调 invoke 对应工具                                     | 通过 list_mcp_tools 列工具 → invoke_mcp_tool(server, tool_name) |

一句话：**MCP 是一种接入工具的方式，接入后 MCP Server 里的工具就变成了 Agent 可用的 Tool**。我这边既有原生内置的 Tool，也有通过 MCP 接入的工具（如 commerce 点单服务），对你调用层面统一都表现为"工具"。
...
npx gitnexus analyze --skip-skills
...
afxp_919502tlLbBsOgxVX9LM6bFVW46UOVqTwUqR
afxp_17b5aaujLO6RE3DD1I6UPm22sTQhSLB4WbI4
