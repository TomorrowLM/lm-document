...
每当AI智能体执行一步操作时，它都会将整个对话历史记录重新发送给LLM。

这包括了系统指令、工具定义，以及它在三轮对话前已经处理过的项目上下文。所有这些信息在每一轮交互中都会被重新读取、重新处理，并重新计费。

每个智能体请求都包含两个根本不同的部分：

- **静态前缀**：在每轮交互中保持不变的部分：系统指令、工具定义、项目上下文和行为指南。
- **动态后缀**：随着每轮交互而增长的部分：用户消息、助手响应、工具输出和终端观测结果。
...
在 Agent / RAG（检索增强生成）语境下，"召回率"指：

**召回率 = 检索到的相关文档数 / 所有相关文档总数**

也就是：所有"应该被找到"的内容里，Agent 实际找到了多少。

| 指标                   | 含义       | 关注点                 |
| :--------------------- | :--------- | :--------------------- |
| **召回率 (Recall)**    | 找全了没有 | 遗漏了多少相关结果     |
| **精确率 (Precision)** | 找对了没有 | 结果里混了多少不相关的 |
|                        |            |                        |

**举个例子**：知识库里有 10 份和"报销流程"相关的文档，Agent 检索后返回了 7 份（其中 5 份相关、2 份无关），则：

- 召回率 = 5/10 = **50%**（漏了一半相关内容）
- 精确率 = 5/7 ≈ **71%**

**为什么召回率重要**：召回率低意味着 Agent "没看到"关键信息，回答就会不完整或答非所问——这是 RAG 场景中最致命的问题，因为检索不到，后续生成阶段再强也无从弥补。
...
- 任务模块化 (Modularization)+精确的信息投喂 (Targeted Feeding)
  ...
  - 约束提示
    - 使用技能/规则约束
- **维护清晰的文档（语义锚点）**
- **切勿在会话中途切换模型。** 缓存是特定于模型的，这意味着在对话中途切换到更便宜的模型需要从头开始重建整个缓存。
...
```
Phase 1 需求分析 → [⛔确认] → Phase 2 技术方案 → [⛔确认] → Phase 3 实现计划 → [⛔确认] → Phase 4 编码实现 → Phase 5 测试验收 → ✅完成
```
...
| 阶段                 | 核心动作                                                    | 产出                              | 门控条件         |
| -------------------- | ----------------------------------------------------------- | --------------------------------- | ---------------- |
| **Phase 3** 实现计划 | writing-plans 生成 → 7 步任务序列 → 影响评估 → 执行方式选择 | 详细实现计划 + 用户选择的执行方式 | 用户确认实现计划 |

#### **Phase 4** 编码实现

| 阶段                 | 核心动作                                     | 产出             | 门控条件 |
| -------------------- | -------------------------------------------- | ---------------- | -------- |
| **Phase 4** 编码实现 | 前置检查 → 按执行方式编码 → TDD 红绿重构循环 | 可运行的业务代码 | —        |
...
UI 验证不通过  → 回到 Phase 4 修复 → 重新 Phase 5
代码审查 Critical → 修复后重新验收 → 审查循环
...
**智能体（Agent）** 和 **智能问答（Q&A）** 的核心区别在于**自主行动能力**：

| 维度 | 智能体（Agent） | 智能问答（Q&A） |
| :--- | :-------------- | :-------------- |
|      |                 |                 |

| **工作模式** | 自主规划、拆解任务、多步执行                         | 单轮问答，一问一答                          |
| ------------ | ---------------------------------------------------- | ------------------------------------------- |
| **工具调用** | 可调用 MCP、Shell、API、文件系统等外部工具           | 不能执行任何工具，仅基于知识库/模型能力回复 |
| **系统交互** | 可读写文件、操作应用、执行命令、访问网络             | 纯文本输出，无系统级操作权限                |
| **状态管理** | 有上下文记忆，可跨步骤追踪任务进度                   | 通常无持久化任务状态                        |
| **适用场景** | 自动化任务（代码审查、文件整理、批量处理、系统配置） | 知识查询、概念解释、方案建议                |
...
![第1页-1](https://image-1304658407.cos.ap-shanghai.myqcloud.com/%EF%BC%88%E6%8F%90%E5%8F%96%E5%9B%BE%E7%89%87%EF%BC%89Claude%20Sub-Agent%20%E5%AE%9E%E8%B7%B5%E6%8C%87%E5%8D%97/20260622151815_f83009.png)

### 特征

| 特征       | 说明                                               |
| ---------- | -------------------------------------------------- |
| 自主运行   | 一旦触发，子代理独立执行多步骤工作流，无需用户干预 |
| 上下文隔离 | 每个子代理拥有独立的上下文窗口，不会污染主会话     |
| 专业化能力 | 通过系统提示词定义专业领域，成为特定领域的专家     |
| 并行执行   | 多个子代理可以同时运行，每个在自己的子进程中       |

#### 2.1 上下文隔离：不污染主对话

这是 Sub-Agent 最基础也最重要的价值。每个 Sub-Agent 运行在独立的上下文窗口中，这意味着子代理繁重的"思考"过程不会消耗主对话的宝贵 Token。

PubNub 的工程团队在实践报告中指出：

> "Agents have their own context window and can provide a summary after doing extensive research to the main agent. That way you can save precious time within the main agent before it has to 'compact'."

#### 2.2 并行处理：大幅缩短时间

Sub-Agent 支持同时运行多个子代理，这是提升效率的关键。

**实际场景**：如果你想分析一个代码库中的重复代码、复杂度过高的函数和死代码，可以同时启动 3 个子代理分别处理，而不是一个接一个地执行。

#### 2.3 专业化分工：让 AI 成为领域专家

每个 Sub-Agent 可以被赋予专门的系统提示词和工具权限，成为特定领域的专家。你可以创建：

- **代码审查员**：专注于代码质量、安全性和可维护性
- **调试专家**：擅长根因分析和最小化修复
- **架构师**：负责技术决策和 ADR 撰写
- **测试生成器**：自动生成单元测试和集成测试
...
通过工具白名单机制，可以为不同的子代理授予不同的权限：

- **只读 Agent**：只有 Read、Grep、Glob 权限，用于代码探索
- **受限 Agent**：可以执行特定命令但不能编辑文件
- **全功能 Agent**：拥有完整工具集，用于实施任务

> "Anthropic's documentation recommends granting only the minimum set of permissions required for each subagent's role"

### 创建（claude）

#### 文件存放位置

Sub-Agent 配置文件存放在以下目录：
...
Sub-Agent 使用 Markdown 文件，包含 YAML 格式的前置元数据和系统提示词：

```yaml
---
name: agent-identifier           # 必填：kebab-case 格式，3-50 字符
description: |                   # 必填：以 "Use this agent when..." 开头
  ...
  user: "[用户请求]"
  assistant: "[响应方式]"
  <commentary>[解释为何触发]</commentary>
  </example>

model: inherit                   # 可选：inherit/sonnet/opus/haiku
color: blue                      # 可选：UI 显示颜色
tools: ["Read", "Grep", "Glob"]  # 可选：工具白名单
---
...
| `description` | ✅    | 触发条件描述，必须包含示例 | 见上方模板              |
| `model`       | ❌    | 使用的模型，默认 inherit   | `sonnet`, `haiku`       |
| `color`       | ❌    | UI 显示颜色                | `blue`, `cyan`, `green` |
| `tools`       | ❌    | 工具白名单，逗号分隔       | `["Read","Grep"]`       |
...
| 颜色          | 适用场景           |
| ------------- | ------------------ |
| 🔵 blue / cyan | 分析、审查类 Agent |
| 🟢 green       | 代码生成类 Agent   |
| 🟡 yellow      | 验证、检查类 Agent |
| 🔴 red         | 安全、审计类 Agent |
| 🟣 magenta     | 创意、转换类 Agent |
...
| 方法       | 命令/操作                  | 说明                     |
| ---------- | -------------------------- | ------------------------ |
| 交互式命令 | `/agents`                  | 打开管理界面，可视化创建 |
| 手动创建   | 创建 `.claude/agents/*.md` | 直接编写配置文件         |
| Meta Agent | 自然语言描述需求           | 让 Claude 帮你生成       |

#### 4.2 实战示例：产研流程

以"做一个个人博客（HTML 版本）"为例，创建四个角色的 Sub-Agent：
...
description: 当用户需要分析产品需求、撰写PRD文档、定义用户故事、排定功能优先级或制定验收标准时使用
model: inherit
color: red
---
```
...
4. 编写完整的 PRD 文档内容，包括：
   ...
   - 验收标准
   - 优先级排序
5. 必须使用 `Write` 工具将完整的 PRD 文档保存到当前工作目录
   - 文件名格式：`prd-<项目名称>.md` 或 `PRD.md`
   - 保存路径：当前工作目录 `./`
6. 保存完成后，输出保存结果确认（包含文件路径）
...
---
name: design-manager
description: 当用户需要设计UI界面、制定设计规范、原型设计、视觉优化、设计系统搭建或用户体验分析时使用
model: inherit
color: ?
---
```
...
4. 定义关键交互的交互行为
5. 指定视觉设计系统（颜色、排版、间距）
6. 创建可供开发者实现的基于文字的线框描述

**输出格式：** 使用文字描述来描述设计。对每个页面/屏幕，使用以下格式：
...
---
name: rd-manager
description: 当用户需要编写代码、调试程序、代码审查、技术方案设计、重构优化或解释代码逻辑时使用
model: inherit
color: ?
---
```
...
---
name: qa-manager
description: 当用户需要编写测试用例、执行测试、Bug追踪、自动化测试、性能测试或测试报告编写时使用
model: inherit
color: ?
---
```
...
你是一名专业的质量保证/测试工程师智能体。你的职责是：

1. 根据 PRD 和需求创建测试计划和测试用例
2. 对已实现的代码执行测试
3. 发现 bug 和问题
4. 验证所有验收标准都已满足
5. 清晰地报告问题并提供复现步骤
...
> ⚠️ **研发场景**：代码审查、单元测试、SQL 优化、文档生成、代码提交
>
> **测试场景**：测试用例、缺陷分析、Mock 数据、兼容性测试

#### 4.3 调用方式

##### 方式 1：自动委派

Claude 会根据子代理的 `description` 自动判断何时调用。

> "帮我审查最近的代码改动" → 自动触发 `code-reviewer`

##### 方式 2：显式调用

直接指定子代理的名字：

> "使用 `code-reviewer` 子代理检查 `src/` 目录"
...
```bash
/agent Explore "分析API路由" & /agent Bash "运行测试" & /agent Plan "设计认证模块架构" &
```
...
| ------------------------ | ------------------ |
| `/agents`                | 打开子代理管理界面 |
| `/agent list`            | 列出所有可用子代理 |
| `/agent [name] "[task]"` | 显式调用子代理     |

| 检查项   | 验证标准                                   |
| -------- | ------------------------------------------ |
| 命名     | 小写、连字符、3-50 字符                    |
| 描述     | 以 "Use this agent when..." 开头，包含示例 |
| 系统提示 | 内容充实（>20 字符），职责明确             |
| 模型     | 有效标识符                                 |
| 颜色     | 有效 UI 颜色值                             |
...
- 优化skill描述信息（name和description）
  - description包括触发场景和约束场景
- skill路由分层
  - 功能区分
- 加入什么时候不能使用进行约束
...
| 周次    | 目标                                          | 核心内容                                                     | 推荐资源                                                     |
| :------ | :-------------------------------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| 第1周   | 建立正确认知                                  | Agent 三要素（思考 + 工具 + 记忆）；CoT/ReAct 基础推理范式；Token、流式输出、结构化 Prompt 规范 | Andrew Ng《AI Agent》公开课 / 李宏毅《Agent 发展脉络》       |
| 第2周   | 跑通带可视化 UI 的最简 Agent（前端特色 Demo） | LangChain JS / Vercel AI SDK Hello World；Next.js 流式对话；前端渲染 Agent 思考、工具调用状态 | Vercel AI SDK 官方文档、LangChain JS 中文教程 + B 站 "肖立新"AI Agent 系列 |
| 第3-4周 | 掌握双栈主流 Agent 框架（Python+JS）          | Python：LangGraph、CrewAI；JS：LangChain JS、Vercel AI SDK；记忆、工具、基础 RAG 集成 | LangGraph 官方文档、CrewAI 中文文档                          |
| 第5-6周 | 做出能用的 Agent                              | 自动周报/竞品搜集/简历助手                                   | 选1-2个真实需求改造                                          |
| 第7周+  | 进阶自由探索：复杂推理 + 多智能体 + 工程部署  | Plan-and-Execute 任务规划、多智能体协作；AutoGen、MetaGPT；Docker 打包、云端部署、密钥安全、日志监控 | AutoGen、MetaGPT 等                                          |
|         |                                               |                                                              |                                                              |
...
| 框架              | 零基础友好度 | 社区活跃度 | 生产可用度 | 典型场景                    | 推荐指数   |
| :---------------- | :----------- | :--------- | :--------- | :-------------------------- | :--------- |
| **CrewAI**        | ★★★★★        | ★★★★☆      | ★★★☆☆      | 快速原型、个人工具          | 零基础首选 |
| **LangGraph**     | ★★★★☆        | ★★★★★      | ★★★★★      | 中大型项目、复杂流程        | 强烈推荐   |
| **LangChain**     | ★★★★☆        | ★★★★★      | ★★★★★      | 通用 Agent 平台、生态最丰富 | 入门首选   |
| **AutoGen**       | ★★☆☆☆        | ★★★★☆      | ★★★★☆      | 多角色协作、企业级应用      | 中后期     |
| **OpenAI Agents** | ★★★☆☆        | ★★★★☆      | ★★★★☆      | 轻量多 Agent                | 辅助使用   |
| **LlamaIndex**    | ★★★☆☆        | ★★★★☆      | ★★★★☆      | RAG+Agent 知识库场景        | 辅助使用   |
...
| 场景                   | 推荐                           | 理由                                                |
| :--------------------- | :----------------------------- | :-------------------------------------------------- |
| 快速原型 / 学习入门    | LangChain 或 OpenAI Agents SDK | 文档最完整，10行代码跑通第一个 Agent                |
| 复杂工作流 / 状态管理  | LangGraph                      | 图结构支持条件分支、循环和检查点                    |
| 多Agent协作 / 企业流程 | CrewAI 或 AutoGen              | CrewAI 角色抽象易对齐业务；AutoGen 事件驱动适合生产 |
| 知识库 + Agent         | LlamaIndex                     | RAG 与 Agent 深度整合                               |
|                        |                                |                                                     |

## 跑通最简单的 Agent

- **核心目标**：复用前端技术栈，完成带可视化交互的对话 Agent，区别于纯命令行 Python 脚本 Demo

  **技术栈组合**：`Next.js 14+` + `Vercel AI SDK` + `OpenAI/DeepSeek API` + TypeScript

  关键落地里程碑

  1. 实现前端流式打字对话 UI，支持实时输出模型回复；

  2. 接入简易外部工具（天气 API、全网搜索 API），完成基础工具调用逻辑；
  ...
  3. 前端差异化亮点
     ...
     - 展示模型思考 loading 动画；
     - 弹窗 / 卡片展示工具调用参数、调用结果；
     - 区分用户消息、模型思考文本、工具返回数据三种 UI 样式；

  产出：可直接浏览器打开的 AI 对话网页，可放入作品集演示。
...
- **难度**：★★☆☆☆
- **必备掌握技能**：工具调用、对话记忆管理、多步骤任务规划、输出自我纠错、向量存储、前后端接口联调
- 分步落地流程
...
1. 竞品信息搜集 Agent：自动爬取竞品官网 / 资讯，整理竞品动态、价格、功能对比文档；
2. 简历优化助手：上传简历 PDF，Agent 结合岗位 JD 优化简历、生成面试高频问答。

## 进阶自由探索

1. **推理范式深挖**：ReAct、Plan-and-Execute、Self-reflection 自校验纠错机制；
2. **多智能体系统**：AutoGen、MetaGPT，实现多角色分工协作（产品、开发、测试智能体）；
3. 工程化落地能力
   - 向量数据库进阶：Pinecone、Milvus 云端向量库；
   - 部署：Docker 容器打包、云服务器部署、环境变量密钥隔离；
   - 监控与优化：日志记录、Token 消耗统计、上下文截断优化、输出安全过滤；
4. **拓展方向**：本地私有大模型 + Agent、浏览器端离线 Agent、AI 工具插件开发。