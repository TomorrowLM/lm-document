...
`js/tsconfig.json` 文件是一个用于配置js/ts项目的 JSON 文件，它可以控制 IntelliSense（导航、智能提示等）和模块解析行为
...
- 当你使用 `pnpm install <package>` 时，pnpm 确实会安装所有 `package.json` 中的依赖，这是**设计如此**的行为。

- Vue packages version mismatch:

          - vue@3.5.24 (D:\work\demo\front\qiankun\node_modules\.pnpm\vue@3.5.24_typescript@4.9.5\node_modules\vue\
  
  vue版本串了，在根node_modules/.pnpm/下移除这个vue@3.5.24
  
- vue2-pc

  - Proxy error: Could not proxy request /white/login from localhost:8002 to http://localhost:3600.
...
MCP（Model Context Protocol）是 Anthropic 2024 年 11 月推出的开源协议，定位是「AI 时代的 USB-C 接口」，用一套标准把大模型（LLM）与外部数据/工具/系统双向连接起来。。其核心功能可归纳为 6 大类：
