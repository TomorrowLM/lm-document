...
`.d.ts` 文件是 **TypeScript 声明文件**，用于提供 JavaScript 库/模块的 **类型定义**，让 TypeScript 能够理解纯 JavaScript 代码的类型结构。

#### 三斜杠指令

**显式依赖声明**：**告诉 TS 编译器“在处理当前文件之前，先把某个声明文件也纳入编译上下文。**早期写多个 `.d.ts` 时很常用，比如例子里，`index.d.ts` 通过 `/// <reference path="./common.d.ts" />` 去引用同目录的 [common.d.ts](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)，确保其中的 `declare global` 等内容在其他文件被解析前就已生效。

最常见的用法是`/// <reference types="..." />`，用于声明对类型定义包（如`@types`包）的依赖

```
/// <reference types="lodash" /> @types/lodash
```
...
1. **避免全局污染**：默认情况下，模块内的内容不会污染全局
2. **明确意图**：如果你想扩展全局，必须显式使用 `declare global`
3. **类型安全**：防止意外的全局变量声明

**注意**

- **`declare` 与 `export` 同级使用，声明文件就需要导入了**
- **在声明文件中 `type` 与 `interface` 也可以不用加 `declare` ，效果相同**

### tip

- 为什么import用了之后，declare module '@/*'无效了
