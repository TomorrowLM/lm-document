...
![img](https://image-1304658407.cos.ap-shanghai.myqcloud.com/%E5%89%8D%E7%AB%AF/%E9%A1%B9%E7%9B%AE%E6%9E%84%E5%BB%BA/20260408141910_6a5993.awebp)
...
- 添加防火墙端口，每个端口通过 frp 去对应本地的端口
  ...
  ![image-20240321003719648](https://image-1304658407.cos.ap-shanghai.myqcloud.com/app/aliyun/20260408142222_6c9f3f.png)
...
- 展示: 服务器公网+webServer.port
  ...
  ![image-20240321003056171](https://image-1304658407.cos.ap-shanghai.myqcloud.com/app/aliyun/20260408142623_f517f7.png)
...
- **重新导出（Re-export）** 指定路径模块的所有导出成员

  - `export * from './init';` - 导出所有命名导出
  ...
  - `export { default } from './init';` - 只导出默认导出

  - `export { specificFunc } from './init';` - 导出特定成员
...
webpack 4 正式版本扩展了此检测能力，通过 package.json 的 "sideEffects" 属性作为标记，向 compiler 提供提示，表明项目中的哪些文件是 “pure(纯正 ES2015 模块)”，由此可以安全地删除文件中未使用的部分。
...
### Webpack 4 与 Webpack 5 核心区别对比

| 对比维度                    | **Webpack 4**                                                | **Webpack 5**                                                | **核心优势（为什么升级？）**                                 |
| :-------------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| **🚀 核心新特性**            | 引入了`mode`配置，支持零配置启动，代码分割需要手动配置`SplitChunksPlugin` 。 | **引入了模块联邦（Module Federation）**，这是最重大的变革，允许多个独立应用之间在运行时共享代码，无需本地安装 。 | **彻底改变了微前端的实现方式**，让代码共享和团队协作变得更加简单高效。 |
| **⚡️ 构建性能与缓存**        | 构建缓存主要依赖`cache-loader`等第三方 loader 实现，配置相对繁琐 。 | **内置了持久化缓存**，通过`cache: { type: 'filesystem' }`即可开启。这能大幅提升二次构建的速度 。 | **开发体验大幅提升**，特别是在大型项目中，二次启动和热更新都快如闪电。 |
| **🌳 Tree Shaking 优化**     | 能够移除未使用的 ES Module 导出，但对带有副作用的代码处理能力有限 。 | **增强了 Tree Shaking 的能力**。能更智能地分析模块的`sideEffects`（可在`package.json`中配置），从而更安全地剔除未使用的代码，甚至对嵌套的 ES Module 也能生效 。 | **打包出来的文件体积更小**，生产环境代码更纯净，加载速度更快。 |
| **⚙️ 配置与插件**            | 需要配置`CommonsChunkPlugin`来处理公共代码，许多优化功能依赖插件（如`uglifyjs-webpack-plugin`）。 | **移除了多个内置插件**（如`CommonsChunkPlugin`），将其功能集成到`optimization`配置中，核心更加简洁。**默认支持压缩 ES6+ 代码** 。 | **配置更简洁，开箱即用的功能更多**，减少了第三方插件的维护成本。 |
| **🔧 Node.js 模块 Polyfill** | 出于对 Node.js 环境的兼容，**默认会为很多核心模块（如`path`、`crypto`）自动提供 polyfill**，增加了打包体积 。 | **不再自动提供 Node.js 核心模块的 polyfill**。如果你在浏览器环境中使用了这些模块，需要手动配置或添加 fallback 。 | **让打包目标更明确（主要面向浏览器）**，避免了冗余代码的自动注入，让开发者自己决定是否需要这些 polyfill。 |
| **📦 资源模块处理**          | 处理图片、字体等资源文件通常需要配置`file-loader`、`url-loader`等 。 | **引入了内置的 Asset Modules**，通过`type: 'asset/resource'`等配置即可处理资源，无需额外 loader 。 | **简化了资源配置**，让配置文件更加清爽，也减少了 loader 依赖。 |
| **🧩 模块解析与类型**        | 支持 JavaScript 和 JSON 模块，对 WebAssembly 的支持处于实验性阶段 。 | **增强了对 JSON 模块的 Tree Shaking**，对 WebAssembly 模块的支持更加稳定，并支持更多模块类型 。 | **更好地拥抱现代 Web 标准**，让开发者更容易集成 wasm 等高性能模块。 |
...
![image-20260124183624708](img/前端/项目构建/image-20260124183624708.png)
...
CDN（Content Delivery Network，内容分发网络）缓存是前端部署中的一项核心加速机制。下面从几个维度讲清楚。

---

#### 一、是什么

CDN 在全球各地部署了边缘节点服务器。当你把前端静态资源（JS、CSS、图片、字体等）部署到 CDN 上时，用户的请求会被智能路由到离他**最近的边缘节点**，直接从该节点的缓存中返回资源，而不是每次都回源到你的源服务器。

```
用户（深圳） → 广州边缘节点（有缓存） → 直接返回
用户（深圳） → 广州边缘节点（无缓存） → 回源拉取 → 缓存 → 返回
```
...
| 问题     | CDN 如何解决                           |
| -------- | -------------------------------------- |
| 物理延迟 | 就近访问，减少网络跳数和 RTT           |
| 源站压力 | 大部分请求被边缘节点拦截，回源比例极低 |
| 带宽瓶颈 | 多节点分担流量，避免源站带宽打满       |
| 可用性   | 单节点故障不影响全局，天然具备容灾能力 |

---

#### 三、前端部署的关键：缓存更新策略

CDN 的强大反过来也带来了一个问题：**资源更新后，用户拿到的还是旧缓存**。解决方式：

##### 核心手段：文件指纹（Hash）

```
旧：bundle.js
新：bundle.a3f8b2c.js  ← 每次构建生成唯一哈希
```

文件名变了，对 CDN 来说就是一个全新的资源，旧缓存自动失效。这是最彻底的方式。

##### 配合手段

| 手段             | 说明                                                         |
| ---------------- | ------------------------------------------------------------ |
| HTML 不做强缓存  | `index.html` 设置 `Cache-Control: no-cache` 或短时效，确保入口文件每次校验 |
| 静态资源做强缓存 | JS/CSS/图片设置 `Cache-Control: max-age=31536000, immutable` |
| CDN 刷新/预热    | 紧急情况手动刷新缓存；大版本发布前预热关键资源               |
| 非覆盖式发布     | 新旧文件共存，HTML 指向新文件，旧文件保留一段时间确保平滑过渡 |
...
```
1. 构建：webpack/vite 生成带 hash 的文件
2. 上传：dist/ 推送到 CDN（或对象存储 + CDN 回源）
3. 引用：index.html 指向带 hash 的 CDN 地址
4. 用户访问：
   - index.html → 每次校验（no-cache）
   - bundle.a3f8b2c.js → 直接命中 CDN 缓存（强缓存）
5. 下次发布：hash 变了，对新用户完全透明生效
```

---

总结：CDN 缓存本质上是用**空间换时间**，用就近节点减少延迟。前端部署的核心在于通过**文件指纹实现缓存的无痛更新**，做到用户端零感知的热发布。
...
- **`main` (CommonJS 形态 - 简装版)**：这是一种“保守”的形态。它把所有的代码打包成一个整体，无论你实际上只用到了其中一小部分功能，整个模块都会被加载进来。它就像一个简装版的大礼包，虽然简单易用，但不够精致，会有不少“冗余”。
- **`module` (ES Module 形态 - 精装版)**：这是一种“开放”的形态。它保留了 `import` 和 `export` 这样的“接口”。现代的打包工具（如 Webpack）非常聪明，它可以通过这些静态的“接口”在打包时进行静态分析，精准地找出哪些代码被真正使用了，然后在最终的 bundle 中把那些未被使用的代码（dead code）“摇晃”（Tree Shaking）掉。这就像一个精装版的模块套装，允许你只带走你需要的部分，非常轻量和高效。

一个同时支持两种环境的包，其配置通常是这样的：

json

```
{
  "name": "my-progressive-package",
  "version": "1.0.0",
  "main": "dist/index.js",        // 入口1: 指向 CommonJS 版本 (ES5 + CJS)
  "module": "dist/index.esm.js"   // 入口2: 指向 ES Module 版本 (ES5 + ESM)
}
```
...
`peerDependencies` 可以翻译为“对等依赖”或“同行依赖”。这个术语在 [npm](https://so.csdn.net/so/search?q=npm&spm=1001.2101.3001.7020) 包管理中用来指代那些不应该由当前包直接安装，而是由项目的使用者安装的依赖包。

- 作用：主要为了解决 **“单例”** 和 **“版本冲突”** 问题。避免在每个依赖包中重复安装相同的依赖，从而减少项目的总体大小。

#### **场景：**

**插件、加载器、框架工具库**

例如，你开发了一个 `awesome-react-button` 组件库。

1. **你的包**：`awesome-react-button`
2. **你的包所依赖的核心库**：`react`

如果你把 `react` 放在 `dependencies` 里，会发生什么？

- 用户安装 `awesome-react-button` 时，npm 会在 `node_modules/awesome-react-button` 目录下再安装一份 `react`。

- 现在，用户的项目结构可能是这样的：

  ```
  node_modules/
  ├── react/                 # 用户项目自己的 react (版本 18.2.0)
  └── awesome-react-button/
      └── node_modules/
          └── react/        # 你的包私有的 react (版本 18.1.0)
  ```

- 这就导致了 **两个不同版本的 `react` 实例**！这会引发巨大的问题，因为 React 在内部依赖于一个全局的单例状态（比如 Hook 的状态、Context）。两个实例同时存在会破坏这个机制，导致无法预料的 bug。

#### **正确的做法：**

在你的 `awesome-react-button` 的 `package.json` 中，将 `react` 声明为 `peerDependencies`：

json

```
{
  "name": "awesome-react-button",
  "version": "1.0.0",
  "peerDependencies": {
    "react": ">=16.8.0",
    "react-dom": ">=16.8.0"
...
- npm/yarn/pnpm 在安装时，会检查用户项目的根目录 `node_modules` 里是否已经安装了 `react` 和 `react-dom`。
- 如果没安装，会给出一个警告。
- 如果安装了，但版本不满足 `>=16.8.0`，也会给出一个警告。
- **关键是**：你的 `awesome-react-button` 包在运行时，会直接使用用户项目中的那一份 `react`，保证了整个应用只有一个 `react` 实例。
...
- ```
  ...
  - 含义是：**使用我 (@lm/shared) 的项目，自己要安装这些包（file-loader 等）**
...
- 提示出现的时机：
  ...
  - 如果不满足，才会在
    ...
    - `peer dep missing: file-loader@^6.2.0, required by @lm/shared@1.0.0`

- **不会在这些动作里自动提示：**

  - 运行 [npm run build](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)
  - 运行 `npm run serve`
  - 打包 / 启动 devServer / vscode 里跑 webpack
  - 这些时候包管理器已经不参与了，所以不会再吐 peerDeps 的 warning

- **但下面这些情况就“不会再提示”了：**

  - 根目录或某个上层包已经安装了一个满足 `^6.2.0` 的 `file-loader`
  - 你之前已经装过一次，有 lockfile + node_modules，之后只是改了 peerDependencies 没有重新跑 install
  - 终端输出太长，你没注意到上面的 [WARN Issues with peer dependencies](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 小段

所以：**就算 vue-demo-pc 确实没装 file-loader，构建阶段也不会自动出现 “peerDependencies 未满足” 的提示**，除非你去重新跑一次 `pnpm install` 并看安装输出。
...
`tsconfig.json` 是 TypeScript 项目的‌**核心配置文件**‌，它主要承担两个关键作用：‌**标识项目根目录**‌和‌**配置 TypeScript 编译器的行为**‌。它可以控制 IntelliSense（导航、智能提示等）和模块解析行为
...
**babel 是一个 JavaScript 编译器（更常称为转译器 transpiler）**

如今大多数的浏览器都支持 es6+的语法和特性，但难免有些旧版本的浏览器是不支持的，为了兼容这些旧版本的浏览器，就有了 babel 这一个 JavaScript 编译器。它能为我们做一些事情：
...
- 查看全部：使用 `npm list` 命令来查看当前安装的包及其版本：

  ```
  npm list --depth=0
  ```

- 查看指定版本：npm view antd version

##### 版本关系查看

```
# 查看 antd 的 peerDependencies (对等依赖)
...
这个输出告诉我们，antd 明确要求的对等依赖是 React 和 React DOM，**它通常不会把 TypeScript 放在这里**。因为 TypeScript 是开发时的类型工具，antd 一般不会强制要求用户必须使用某个版本的 TypeScript 

##### **查看包信息**

`npm info`（别名：`npm view`）是 npm 的一个**查看包信息**的命令。它可以让你**在不安装包的情况下**，查看该包在 npm registry 上的所有元数据信息。
...
# 查看作者
npm info react author

# 查看发布时间
npm info react time

# 查看某个具体版本的信息
npm info react@17.0.2
```
...
```
# 查看所有 react 17.x 的版本
npm info react versions | grep '17\.'

# 查看包的大小
npm info react dist.unpackedSize
```
...
‌**在webpack项目中可以使用CommonJS导入导出模块，主要是因为现代JavaScript构建工具如Webpack具备模块互操作性功能，允许在项目中**

**混合使用CommonJS和ES6模块语法**‌。具体来说，当使用`import`语法引入一个使用CommonJS导出的模块时，构建工具（如Webpack）会在构建

过程中自动将CommonJS模块转换为一个可以被ES6 `import`语法识别的格式，从而实现了两种模块系统的无缝集成‌
...
> 源码中既有esm又有cjs的模块存在，存在commonjs与es6混用的情况，并且需要双模式发布

### tsc区别

**tsc 能编译，但不能打包CJS 模块**

| 特性             | TypeScript 编译器 (tsc)      | Rollup                       |
| :--------------- | :--------------------------- | :--------------------------- |
| **主要职责**     | TypeScript → JavaScript 转换 | JavaScript 模块打包          |
| **输入格式**     | TypeScript/JavaScript 文件   | ES6、CommonJS、AMD 等模块    |
| **输出格式**     | JavaScript (多种模块格式)    | 优化的打包文件 (ESM、CJS 等) |
| **Tree Shaking** | 有限支持                     | 优秀支持                     |
| **代码分割**     | 不支持                       | 支持                         |
| **插件系统**     | 有限 (通过 tsconfig)         | 丰富的插件生态               |
...
当使用 `preserveModules: true` 时，Rollup 会保持原有的模块结构，但会根据你配置的 `output.format` 将每个模块包装成特定的格式。

不同格式的表现：

```
// 如果 format: 'es' - 保持 ES 模块
export function foo() {}
...
exports: 'named',        *// 以具名导出形式输出，消除 Rollup 警告*
```
...
- tslib
  ...
  ```
  直截了当：tslib 是 TypeScript 官方的运行时辅助函数库（runtime helpers）。当 TypeScript 编译器要把某些特性（继承、装饰器、async/await、展开运算等）转换成兼容旧环境的 JS 时，会用到一堆“helper”函数。启用 importHelpers 后这些 helper 会从 tslib 中引用，而不是每个文件内重复生成，能显著减小产物体积并统一 runtime。
  
  核心要点（中文）：
  
  目的：把 TypeScript 编译产生的重复 helper（比如 __extends, __assign, __awaiter 等）抽到一个共享模块，避免每个文件都内联同样代码，从而减少 bundle 大小并提高缓存效率。
  常见 helper：__extends, __assign, __rest, __decorate, __param, __metadata, __awaiter, __generator, __importDefault, __importStar, __createBinding, __values, __read, __spread, __spreadArrays 等。
  如何启用（tsconfig）：在 compilerOptions 中启用：
...
- cross-env NODE_ENV=production.qiankun无效，打印的是production

  原因是：**cross-env 设置成功了，但被 vue-cli 又重置了一次**。

  - 你的脚本现在是：
    `cross-env NODE_ENV=production.qiankun && vue-cli-service build --mode production`
  - vue-cli 的行为：执行 `build` 命令时，会强制把 [process.env.NODE_ENV](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 设为 `"production"`，所以你后面在代码里 [console.log(process.env.NODE_ENV)](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 看到的是 `"production"`，不是 `"production.qiankun"`——这不是 cross-env 失效，而是被覆盖了。
...
@use 只让当前文件能用被引入模块的内容，不会自动再导出；@forward 会把被引入模块的变量/mixin 再“转发”出去。你的入口通过 Vite/webpack 的 `additionalData` 注入的是 `@use "@lm/shared/assets/styles/scss/index.scss" as *;`，所以：

- 如果 `index.scss` 内部用 `@use`，变量只在 `index.scss` 里可见，注入它的上层文件拿不到 `$text`，于是报未定义。
- 改成 `@forward` 后，`index.scss` 相当于聚合/出口文件，把 `themes/index.scss` 等里的变量重新导出，注入它的上层就能看到 `$text`。因此在 [index.scss:1-8](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/072586267e/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 用 `@forward` 是对的。

简单记法：想“消费”用 `@use`，想“转给别人用”在聚合文件里写 `@forward`。
...
> 引用@lm/shared/assets/styles/scss/index.scss样式文件没有生效

`@lm/shared` 之前标了 `"sideEffects": false`，Webpack 会把它当成“纯模块”，如果某个文件没有导出值，就可能被 tree‑shaking 直接裁掉。而 SCSS/CSS 文件本身就是“没有导出的副作用文件”，它的作用是注入全局样式，**所以会被错误地移除**

把 @lm/shared 的 package.json 改成：
...
这会告诉 Webpack：**样式文件都有副作用，必须保留**。

#### pnpm

##### 版本问题

1. React 类型不一致问题：package.json中的@types/react是17，工作区中实际被解析到的 `@types/react` 版本（错误提示里是 @types/react@18）不匹配，导致 JSX 元素类型不兼容（`Provider` 的实例类型与当前 `@types/react` 的 JSX 定义冲突）。

   - 查看版本依赖关系

   - pnpm版本锁定（只在 workspace 根生效）
     ...
     其他项目用18会受影响，把 `pnpm.overrides` 放在 workspace 根部，所有子包都会统一用 React 17 的类型。

     用根目录 `pnpmfile.cjs` 做条件覆盖
...
2. Vue packages version mismatch:
   ...
   ```
   [shared]setOutputConfig name: vue-demo-pc
    ERROR  Error: 
   
           Vue packages version mismatch:
   
           - vue@3.5.29 (D:\work\demo\front\qiankun\node_modules\.pnpm\vue@3.5.29_typescript@4.9.5\node_modules\vue\index.js)
           - vue-template-compiler@2.6.14 (D:\work\demo\front\qiankun\node_modules\.pnpm\vue-template-compiler@2.6.14\node_modules\vue-template-compiler\package.json)
...
- 临时解决：vue版本串了，在根node_modules/.pnpm/下移除这个vue@3.5.24

- 解决

  **pnpm 的 packageExtensions 配置**，作用是**“补齐某些依赖包缺失的 peerDependencies”**，让 pnpm 在解析依赖时按你指定的规则去约束版本。
  ...
  - 强制告诉 pnpm：
    **`vue-template-compiler@2.6.14` 必须依赖 `vue@2.6.14`**
  - 这样可以避免它在 Vue2 项目里被错误地配上 Vue3，导致 “Vue packages version mismatch”。

  简单理解：
  ✅ **这是“补依赖声明”，不是升级/降级包**
  ✅ **它让 pnpm 更严格地锁定 Vue2 与 compiler 的匹配关系**
  ✅ **能减少 Vue2/Vue3 依赖混乱的概率**
...
#### 背景
- 主应用在页面某个模块通过 `loadMicroApp` 动态挂载子应用。
- 目标：**子应用内部路由跳转不影响主应用地址栏**；支持从主应用传入业务路由 `initialPath`。

#### loadMicroApp 与 registerMicroApps 的关系

- 同一个子应用，如果已经通过 `loadMicroApp` 实例化运行，`registerMicroApps` 对该实例不再起作用（不会再被路由激活/卸载）。
- 你可以同时在主应用里注册其他子应用（依赖路由激活），但针对同一个子应用请只选一种方式（避免重复挂载与冲突）。
...
export default function AnBlock() {
  const ref = useRef<HTMLDivElement>(null);
  const microRef = useRef<MicroApp>();

  useEffect(() => {
    window.__AN_ADMIN_INITIAL_PATH__ = '/ai/org?iframePageType=dialog';
    const micro = loadMicroApp({
      name: 'an-admin',
      entry: __DEBUG__ ? '//localhost:8000/an' : `${window.location.origin}/an`,
      container: ref.current!,
      props: { initialPath: '/ai/org?iframePageType=dialog' },
    });
    microRef.current = micro;
    return () => micro.unmount();
  }, []);

  const gotoRisk = () => microRef.current?.update?.({ initialPath: '/risk/list' });

  return (
    <div>
      <button onClick={gotoRisk}>跳转子应用风险列表</button>
      <div id="anIframe" ref={ref} style={{ minHeight: 600 }} />
    </div>
  );
}
```

- 配置项：在 [config/config.ts](config/config.ts) 中，`history: { type: useMemoryHistory ? 'memory' : 'hash' }`。
- 启用方式：通过环境变量 `USE_MEMORY_HISTORY=true`。
  - 开发：`pnpm dev:memory`
  - 生产构建：`pnpm build:memory`
- 启用后，子应用内部的 `push/replace` 只作用于内存路由，宿主地址栏保持不变。

#### 子应用

- 路由前缀 `base: '/an'`，资源路径 `publicPath` 与构建产物目录注入保持一致。
- 在 qiankun 场景下，`app.ts` 的 `mount/update` 会调用 `navigateToInitialPath`：
  - 解析 `initialPath`，支持 `#/xxx` 或 `/xxx`，会剥掉 `/an` 前缀。
  - 使用路由跳转到目标页面。
- 如需完全不改宿主地址栏：将子应用切换为 **memory history**（见下）。

##### 子应用入口示例（app.ts 片段）
```ts
export const qiankun = {
  async mount(props: any) {
    navigateToInitialPath(props?.initialPath);
  },
...
function navigateToInitialPath(path?: string) {
  const targetPath = normalizeInitialPath(path);
  if (targetPath && targetPath !== history.location.pathname) {
    history.replace(targetPath); // memory history 时不会改宿主地址栏
  }
}
```
...
##### 使用 USE_MEMORY_HISTORY 的注意点 / 潜在缺点

- 浏览器前进/后退无法感知子应用内的路由栈（因在内存中），回退只会作用于主应用。
- 子应用页面刷新后，会回到初始路由（依赖 `initialPath` 或 `__AN_ADMIN_INITIAL_PATH__`），无法从地址栏恢复具体子页面。
- 深链直达（复制地址栏分享）不可用；需要依赖主应用逻辑或参数把目标路由重新传给子应用。

##### loadMicroApp 方式的注意点 / 潜在缺点
- 子应用被卸载/重新挂载时，内部状态会丢失（除非你在主应用或全局存储中持久化）。
- 多次挂载同一子应用可能引入额外资源开销和首屏时延（相对 registerMicroApps 的预加载策略）。
- 需要手动管理生命周期（`microApp.unmount()` / `update()`），不走路由自动激活。
- 如主应用容器尺寸/滚动策略处理不好，可能出现子应用内部滚动与外层滚动冲突。

#### 常见问题
- **为什么仍然看到 `/an/xxx`？**
  - 如果未启用 `USE_MEMORY_HISTORY=true`，hash 路由的跳转会写入地址栏。
  - `registerMicroApps` 模式下，首次进入必须让地址栏命中 `activeRule`（如 `/an`）才能激活子应用，这是框架要求；后续内部跳转可用 memory history 避免再改地址。
- **主应用如何二次跳转子应用页面？** 使用 `micro.update({ initialPath })`。
- **子应用独立运行？** 在本地/独立部署时可继续用默认 `pnpm dev` / `pnpm build`（hash 模式）。
...
container.contentWindow?.postMessage({ type: 'USER_INFO', data: user }, '*');
...
必须校验 e.origin 防止恶意消息
子应用卸载时需 removeEventListener，否则造成内存泄漏
qiankun 是单实例（同时只激活一个子应用）时 postMessage 足够简单；多实例同时激活时 initGlobalState 更清晰，因为子应用能通过 onGlobalStateChange 拿到当前最新状态而无需额外处理
...
> 导致 404 的核心原因是 [__webpack_public_path__](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 没被及时改成 qiankun 注入的地址。微应用在被主应用加载时，qiankun 会通过 [window.__INJECTED_PUBLIC_PATH_BY_QIANKUN__](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 注入正确的静态资源根路径，但如果入口文件没有在最开始就引入 [public-path.js](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)，这个值就不会覆盖 webpack 默认的 publicPath，懒加载的 chunk 仍然会去主应用的域名（比如 `http://localhost:3500`）拉取，结果主应用根本没有这个文件，就返回了 404。把 `public-path` 提前引入后，webpack 会用正确的子应用地址（如 `http://localhost:8002`），异步 chunk 才能加载成功。
...
![image-20251227154640613](img/前端/项目构建/image-20251227154640613.png)

加入之后

![image-20251227154722004](img/前端/项目构建/image-20251227154722004.png)
...
使用 qiankun 微前端的子应用中 element-ui 的 icon 图标不显示的问题（单独打开应用 icon 显示没问题，在 qiankun 中打开不显示）

经过搜索资料，element-ui 图标 icon 使用的 是 .woff .ttf 格式的字体图标，浏览器加载后以 <style> 标签的方式引入的，且 **.woff 的引入方**

**式为相对路径引入**，因为 qiankun 框架的主应用地址和子应用地址不一样，导致加载 .woff 文件地址错误，所以图标不显示
...
解决：[微应用打包之后-css-中的字体文件和图片加载-404](https://qiankun.umijs.org/zh/faq#%E5%BE%AE%E5%BA%94%E7%94%A8%E6%89%93%E5%8C%85%E4%B9%8B%E5%90%8E-css-%E4%B8%AD%E7%9A%84%E5%AD%97%E4%BD%93%E6%96%87%E4%BB%B6%E5%92%8C%E5%9B%BE%E7%89%87%E5%8A%A0%E8%BD%BD-404)
...
- react-app
  ...
  根路由资源使用 '/'    APP_ROUTER_BASE: "/", *// 微应用路由根路径*
...
- 访问：http://localhost:8080/qiankun/react-app/#/login
...
#### 本地能访问接口，部署上去请求打到前端静态页上了

> 本地能访问接口，部署上去You need to enable JavaScript to run this app



这个报错本质上不是代码问题，而是“请求打到前端静态页上了”。

你现在的 [getUserInfo](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 是这样调的：

- 请求工具基地址：[APP_PROXY_API](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)
- 在 react 子应用生产环境里：`APP_PROXY_API = '/react-app/api/'`（见 [.env.js](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 和 [.production.qiankun.js](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)）
- [getUserInfo](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 调用：[request.get("/common/userInfo")](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)
- 实际发出的 URL：[/react-app/api/common/userInfo](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)

本地能通，是因为开发环境有 devServer 的 `proxy + pathRewrite`，把
[/react-app/api/common/userInfo](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) → [/common/userInfo](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 转到你在 [app.js](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 起的 Node 服务上。

但你部署之后，如果服务端（Nginx/网关） **没有配置 [/react-app/api/](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 这条反向代理/重写**，那么：

**解决思路（2 选 1）：**

1. **按现在的前端配置，补上服务端代理（推荐）**

   在 Nginx/网关里增加类似配置（示意）：

   这样前端的 [/react-app/api/common/userInfo](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 就能被转到 Node 后端的 [/common/userInfo](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)（[getRoutes.js](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 里正是这么挂的）。

2. **改前端的 [APP_PROXY_API](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)，直接指向后端地址**

   如果你线上没有做反向代理，而是让浏览器直接调后端端口，可以把生产环境改成：

#### Uncaught SyntaxError: Unexpected token '<'

现在线上实际请求：

- 实际：`http://localhost:81/qiankun/child/js/src_view_learn_hooks_useState_index_jsx.xxx.js`
- 预期：`http://localhost:81/qiankun/child/react-app/js/src_view_learn_hooks_useState_index_jsx.xxx.js`

关键点有两个：

1. 子应用打包配置（react-demo）

   在 [.production.qiankun.js](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 里：

   shared 的 WebpackBaseBuilder 会用 [Build_Path](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 作为 `output.publicPath`：

   所以子应用自己打出来的静态资源前缀是 [/qiankun/child/react-app/](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)，这部分是对的。

2. 主应用 qiankun 注册 entry 写法（当前有问题）

   在 [main.tsx](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 里，react 子应用注册是：

   qiankun 内部会把这个 [entry](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 当成 HTML 入口 URL 来解析：

   - 如果是

     /qiankun/child/react-app，它会认为：

     - base path 是 `/qiankun/child/`
     - 文件名是 [react-app](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)

   - 然后用这个 base path 去设置 [window.__INJECTED_PUBLIC_PATH_BY_QIANKUN__](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)，导致 Webpack 的运行时 publicPath 被覆盖成 `/qiankun/child/`。

   这就是为什么浏览器最终去请求 `/qiankun/child/js/...`，少了 [react-app](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 这一段。

我已经帮你把主应用的 entry 改成带 `/` 结尾：

- [main.tsx](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 中现在是：

这样 qiankun 解析时会把 [/qiankun/child/react-app/](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 整个当成目录，注入的 publicPath 也是 [/qiankun/child/react-app/](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)，就和子应用打包出来的路径对齐了，访问地址会变成你期望的：
...
在 Vue CLI 项目中，我们可以通过 vue.config.js 文件来自定义 Webpack 配置。Vue CLI 提供了两种主要方式：

- 使用 `configureWebpack` 函数形式处理环境相关的配置和动态配置

- 使用 `chainWebpack` 精确修改特定的加载器规则和插件选项

  

- configureWebpack，提供一个对象或函数，与原有配置进行合并

  - 第三方插件只认“纯 webpack 对象”

    例如 `webpack-bundle-analyzer`、一些企业内部的 webpack 配置工厂，它们**内部用 `Object.assign` / 深度合并**，**只能吃普通对象**，不接受 `webpack-chain` 实例

  - 一口气替换整个大模块
    想直接把 `resolve.fallback`、`output.library`、`externals` 整块换掉
...
```
项目根目录控制台执行 vue inspect > webapck.config.js（webapck.config.js名字随便起就行）
执行完 ，根目录会多出一个webapck.config.js 的文件，他就是vue最后生成的webpack配置
```
...
- [tslint-loader](https://github.com/wbuchwalter/tslint-loader)跟[tslint](https://github.com/wbuchwalter/tslint-loader)：我想你也会在.ts .tsx文件 约束代码格式（作用等同于eslint）
...
`unplugin-auto-import/vite` 的作用：在 Vite 构建/开发阶段自动帮你“补齐 import”，让你在源码里直接用某些 API（如 `ref`、`computed`、`ElMessage`、`useRouter` 等）而不用每个文件都手写 [import ... from ...](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/072586267e/resources/app/out/vs/code/electron-browser/workbench/workbench.html)。
...
- `imports: ['vue', ...]`：声明要自动导入哪些库的哪些导出（`'vue'` 代表常用 API 全量自动导入）。
- `resolvers: [...]`：按需把特定 UI 库（如 Element Plus）的 API 也纳入自动导入。
- `dts: ...`：生成类型声明文件的路径（你这里是 `typings/auto-imports.d.ts`）。
- `eslintrc.enabled`：生成 eslint globals 配置，避免 eslint 报“未定义变量”。
...
> process.env.NODE_ENV内容都不一样
>
> Vue CLI 在执行 serve/build 时会把 process.env.NODE_ENV 强制设置为 'development' 或 'production'，所以你用 cross-env 设置自定义的 NODE_ENV（比如 "dev" 或 "dev.qiankun"）会被 CLI 覆盖，造成不同地方看到的值不一致。
>
> 推荐做法（简短、直接可执行）：不把语义环境放在 NODE_ENV，使用 Vue 推荐的 mode 或自定义以 VUE_APP_ 前缀的环境变量。
...
引入的包中APP_ENV和vue.config中的APP_ENV不一致，怎么解决

原因（简短）：

- 不同上下文/时机读取 process.env 会不一致：vue.config.js 在 Node 的构建进程中运行，能直接读到 cross-env 设置的 APP_ENV；但你的 shared/getEnv 可能在模块初始化时就读取 env（或被打包到前端），或者在不同进程/时间点执行
...
将 Vite + React 项目中的 SVG 封装成组件，不仅“好”，而且是目前**最推荐、最现代化且最高效的工程化方案**。

相比于传统的 `<img src="logo.svg" />` 引入方式，将 SVG 作为 React 组件使用能带来极大的开发体验和性能

| 维度         | 使用SVG组件                     | 直接使用`<img>`或内联                  |
| :----------- | :------------------------------ | :------------------------------------- |
| **可维护性** | ✅ 集中管理，一处修改全局生效    | ❌ 每个使用处都需要单独修改             |
| **性能**     | ✅ Tree Shaking可剔除无用图标    | ❌ 可能一次性加载所有图标（如IconFont） |
| **类型安全** | ✅ TypeScript可校验图标名称      | ❌ 字符串拼写错误难发现                 |
| **CSS控制**  | ✅ 完全控制fill/stroke/大小/动画 | ❌ `<img>`模式下无法修改颜色            |
| **分包加载** | ✅ Vite自动代码分割，按需加载    | ❌ Spritesheet可能包含大量无用图标      |
| **SSR兼容**  | ✅ 服务端可渲染                  | ✅ 部分方案（如Spritesheet）需特殊处理  |

**如果不封装成组件，为什么未使用的图标代码在打包时不会被剔除**

如果不将 SVG 封装成组件，而是采用传统的 `<img src="..." />` 或 CSS `background-image` 的方式引入，未使用的图标代码之所以无法被剔除，核心原因在于**构建工具（如 Vite/Rollup）对“静态资源”和“JavaScript 模块”的处理机制完全不同**。

我们可以从以下三个层面来深入理解：

**📦 1. 打包工具的认知差异：JS 模块 vs 静态资源**

- **封装成组件时（JS 模块）**：当你把 SVG 变成 React 组件并通过 `import MyIcon from './my-icon.svg?react'` 引入时，Vite 会把它当作一个标准的 JavaScript/ES 模块。构建工具（Rollup）能够完美地分析你的 JS 代码依赖树。如果你定义了但从未在页面中渲染 `<MyIcon />`，打包工具就能精准识别出这个变量是“死代码”，从而通过 Tree-shaking（摇树优化）技术将其彻底剔除。
- **作为图片引入时（静态资源）**：当你使用 `<img src="./my-icon.svg" />` 或者通过 CSS 引入时，`.svg` 文件会被 Vite 视为纯粹的**静态资源（Asset）**。它的工作仅仅是把这个文件原封不动地复制到打包后的输出目录（比如 `dist/assets/my-icon.a1b2c3.svg`），并返回一个文件路径字符串。打包工具**不会、也无法去解析 HTML 模板或 CSS 文件里的字符串路径**，判断你到底有没有在页面上真正用到这张图。对它来说，只要你在代码里写了这个路径，它就认为你需要这个文件。

**🔗 2. 动态引用的“黑盒”效应**

即使你没有直接在 JSX 里写 `<img>` 标签，但只要你存在类似以下的动态引用行为，打包工具就会因为无法准确预测运行时的结果，而选择保守策略：

```
// 假设你把一堆图标放在一个数组里
const iconList = ['home.svg', 'user.svg', 'setting.svg']; 
// 哪怕你只渲染了 iconList[0]，打包工具也不敢轻易删掉后面两个
```

在这种情况下，为了防止程序运行时找不到文件报错，Vite 会把 `iconList` 里涉及到的所有 SVG 文件全部打包进产物中。

**🎨 3. CSS 中的背景图更难被优化**

如果你的图标是通过 CSS 类名来引用的（例如 `.icon-home { background: url('./home.svg') }`），CSS 文件本身通常是被当作一个整体字符串或独立的 CSS AST（抽象语法树）处理的。除非使用了非常激进且配置复杂的 CSS 专用 Tree-shaking 插件（如 PurgeCSS 等），否则常规的 JS 打包流程根本无法触及 CSS 内部引用的资源，自然也就无法剔除这些未被使用的图标。

**总结来说：**
只有当 SVG 变成了 JS/TS 模块（即封装成组件），它才能进入打包工具的“依赖分析雷达”里，享受 Tree-shaking 带来的体积红利。而作为静态图片或背景图，它们只是被搬运的“行李”，打包工具并不知道哪些行李是你真正要带上飞机的，所以只能全部打包带走。
...
https://blog.csdn.net/weixin_60941411/article/details/135538432

Windows在换行的时候，同时使用了回车符CR和换行符LF，即[CRLF](https://so.csdn.net/so/search?q=CRLF&spm=1001.2101.3001.7020); 而项目仓库中默认是Linux环境下提交的代码，文件默认是以LF结尾的

![img](https://i-blog.csdnimg.cn/blog_migrate/b3bff44424c58002e37e58b2aa74404c.png)

#### 解决

- vscode：在vscode设置中，在搜索栏中搜索prettier，找到End Of Line 这个选项设置为 `auto`

  <img src="https://i-blog.csdnimg.cn/blog_migrate/a75547780079ee38bef9a2af32d63ef4.png" alt="img" style="zoom:33%;" />
...
- Windows在换行的时候，同时使用了回车符CR和换行符LF，即CRLF; 而项目仓库中默认是Linux环境下提交的代码，文件默认是以LF结尾的，因此，文本文件在不同系统下创建和使用时就会出现不兼容的问题，当我用windows电脑git clone代码的时候，若我的autocrlf(在windows下安装git，该选项默认为true)为true，那么文件每行会被自动转成以CRLF结尾，若对文件不做任何修改，pre-commit执行eslint的时候就会提示你删除CR。

  Git 提供了一个“换行符自动转换”功能。这个功能默认处于“自动模式”，当你在签出文件时，它试图将 UNIX 换行符（LF）替换为 Windows 的换行符（CRLF）；当你在提交文件时，它又试图将 CRLF 替换为 LF。它一方面保持仓库内文件的一致性（UNIX 风格），一方面又保证本地文件的兼容性（Windows 风格）。
...
在 SCSS 中，下划线（_）通常用于命名私有（partial）文件。**这些文件不会被单独编译**，除非被导入到其他 SCSS 文件中。这样做的好处是可以将大文件拆分成多个小文件，从而更容易管理和重用代码。注意，在导入私有文件时，不需要写文件的扩展名，也不需要前缀下划线。SCSS 会自动寻找匹配的文件名。

```
styles/
├── main.scss        # 主文件，会被编译成 main.css
├── _variables.scss  # 变量定义，不需要独立编译
├── _mixins.scss     # 混入定义，不需要独立编译
└── _header.scss     # 头部样式，只被 main.scss 引入
```

当你创建 `_variables.scss`，这个下划线意味着：

- **这个文件不会被编译成 `_variables.css`**
- 它只能被main.scss通过 `@import` 或 `@use` 引入使用
...
当你编译上面的代码时，SCSS 会自动找到并导入 `_variables.scss` 文件，**并将其内容包含在最终生成的 main.css 文件中。**
...
### ~~10.2 Tailwind + rem 自适应~~

基于 375 设计稿 + Tailwind CSS v4，通过 `amfe-flexible` + `postcss-pxtorem` 实现全栈 rem 自适应。

**工作原理：**

```
375 设计稿 px 值 → postcss-pxtorem 转为 rem → 浏览器按 html font-size 计算实际尺寸
```

**配置步骤：**

**① 入口引入 flexible（`src/main.tsx` 最顶部）：**

```ts
import 'amfe-flexible'
```

> 原理：动态设置 `document.documentElement.style.fontSize = (clientWidth / 375) * 37.5 + 'px'`，375 屏下 `1rem = 37.5px`
...
module.exports = {
  plugins: {
    'postcss-pxtorem': {
      rootValue: 37.5,           // 375 / 10，与 flexible 基准一致
      unitPrecision: 5,
      propList: ['*'],
      replace: true,
      mediaQuery: false,
      minPixelValue: 0,
      exclude: /node_modules/i,  // 不转换第三方库（antd-mobile 等）
    },
  },
}
```

**③ Tailwind v4 自定义 spacing（`src/index.css`）：**

```css
@import "tailwindcss";

@theme {
  --spacing-1: 0.26667rem;   /* 10px / 37.5 */
  --spacing-2: 0.53333rem;   /* 20px */
  --spacing-3: 0.8rem;       /* 30px */
  --spacing-4: 1.06667rem;   /* 40px */
  --spacing-5: 1.33333rem;   /* 50px */
  --spacing-6: 1.6rem;       /* 60px */

  --text-xs: 0.32rem;        /* 12px */
  --text-sm: 0.37333rem;     /* 14px */
  --text-base: 0.42667rem;   /* 16px */
  --text-lg: 0.48rem;        /* 18px */
  --text-xl: 0.53333rem;     /* 20px */
}
```

> 也可不覆盖，直接依赖 `postcss-pxtorem` 全局转换，非常规间距使用任意值语法 `p-[15px]`

> **为什么用 rem 而不是 px？** `amfe-flexible` 已在入口处根据屏幕宽度动态设置 `<html>` 的 `font-size`（375 屏下 `1rem = 37.5px`），浏览器最终渲染尺寸由 rem 值与根字体相乘得出。例如 `--text-base: 0.42667rem` → `0.42667rem × 37.5 ≈ 16px`，在 414 屏上 `font-size` 自动放大到 `414/375×37.5 = 41.4px`，实际渲染 ≈ `0.42667 × 41.4 ≈ 17.66px`，实现等比缩放。
...
```tsx
// Tailwind 类名 — px 会被 postcss-pxtorem 自动转 rem
<div className="p-4 text-sm gap-2">内容</div>

// 任意值语法 — 同样自动转换
<div className="p-[15px] text-[13px]">内容</div>

// 手写 CSS / Less — 同样自动转换
<div style={{ padding: '16px', fontSize: '14px' }}>内容</div>
```
...
| ------------- | ------------------------- | ---------------------------------- |
| 设计稿宽度    | 375px                     | 基准宽度                           |
| flexible 基准 | `1rem = 37.5px`（375屏）  | `font-size = (width / 375) × 37.5` |
| rootValue     | `37.5`                    | 375 / 10，与 flexible 一致         |
| 第三方排除    | `exclude: /node_modules/` | 防止 antd-mobile 被误转            |
| Tailwind v4   | `@tailwindcss/vite` 插件  | 替代 v3 的 postcss 插件            |
...
> 误解：在项目根文件引入样式就行
>
> - 直接在入口 JS `import '@lm/shared/assets/styles/scss/index.scss'` 只是把编译好的 CSS 打到 bundle 里，Sass 变量/mixin 不会“继续传递”到后续被编译的 `.scss` 文件。这是因为 Sass 编译时作用域隔离，JS 里的 CSS 引入已经是产物，不包含编译期符号。
> - 想让每个.scss文件都能直接用$text/mixin，有两条路：
>   1. 在 `sass-loader` 配置里用 `additionalData: '@use "@lm/shared/assets/styles/scss/index.scss" as *;'`（你在 shared 的 webpack.base.js 已经加了，确保 React 项目确实复用这份 builder；Vite 也要配 `css.preprocessorOptions.scss.additionalData`）。
>   2. 或者在每个需要的 `.scss` 顶部写 `@use "@lm/shared/assets/styles/scss/index.scss" as *;`。
...
比如在代码中使用了 ES6 的箭头函数，但是这种写法会在 IE 浏览器中报错，为了让代码能在 IE 中运行，就需要将代码编译成 IE 支持的写法，这就是 babel 的主要工作。
...
- TypeScript 默认会自动加载 `node_modules/@types/*` 下的类型，

- **如果**你在 [tsconfig.json](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 的 `compilerOptions.types` 中显式列出了某些包，TypeScript 只会加载那个列表里的类型，其他 `@types/*` 将被忽略。

  ```
  "types": ["@lm/lint", "lodash", "node"]
  ```
...
- baseUrl
  ...
  `baseUrl` 告诉 TypeScript 非相对导入（即不以 `./` 或 `../` 开头的**导入**）从哪个目录开始解析模块路径。它是编译器/IDE 的解析基准目录，不直接改变运行时行为。

  **常见取值**：通常设为 `.`（项目根）或 `src`（源码根）。

  - 例如：如果 `baseUrl: "."`，则 `import x from "src/foo"` 解析到 `<projectRoot>/src/foo`。
  - 如果 `baseUrl: "src"`，则 `import x from "components/foo"` 解析到 `<projectRoot>/src/components/foo`。

  **只影响 TypeScript/IDE，不自动影响打包器**：`baseUrl/paths` 只让 `tsc` 和编辑器（TS Server）能解析模块；运行时或打包（webpack/vite）也需要对应的别名配置（例如 webpack 的 `resolve.alias`、Vite 的 `resolve.alias`

- target：指定 TypeScript 编译后输出的 JavaScript 语言级别（即“向下转换/降级”到哪个 ECMAScript 版本），不影响模块系统（CommonJS / ESM）。可选值：
...
```json
{
  ...
  "compilerOptions": {
    ...
    "checkJs": true,                                  /* 是否检查和报告 JavaScript 文件中的错误 */
...
`extends` 常用于共享规范（风格/paths/target）；`references` 用于把多个包当作编译单元进行增量构建和类型共享。
...
- `references`
  ...
  被 `references` 引用的子项目必须在它自己的 `tsconfig` 中设置 `"composite": true`
...
`.d.ts` 文件是 TypeScript 中的声明文件，用于提供类型信息。它们对于让 TypeScript 编译器**理解第三方库或现有 JavaScript 代码**的类型至关重要

#### 顶级声明 declare

`declare` 是 TypeScript 中的一个关键字，用于**声明**变量、函数、类、模块、接口等的类型信息。以 declare 声明的变量和模块后，其他地方不需要引入，就可以直接使用了。注意我们需要在配置文件下，引入声明文件
...
在 `.d.ts` 文件中声明全局变量或函数，这些声明将影响整个项目。
...
declare module 'myLibrary' {
  export function myFunc(arg: number): void;
  export const myVar: string;
}
```

这声明了一个模块 `myLibrary`，并提供了模块内导出的函数 `myFunc` 的类型信息。
...
1. **语义不同**：
   ...
   - `namespace`：逻辑分组，类似于 Java 的包
   - `module`：物理模块，对应于实际的文件/包

2. **现代 TypeScript**：

   - `namespace`：逐渐被 ES6 模块取代
   - `module`：仍然重要，用于声明第三方模块类型

3. **使用建议**：

   在大多数现代 TypeScript 项目中，**优先使用 `declare module` 为外部包提供类型定义**，而 `declare namespace` 主要用于处理遗留代码或特殊的全局类型扩展场景

#### 模块化

##### 三斜杠指令

**显式依赖声明**：**告诉 TS 编译器“在处理当前文件之前，先把某个声明文件也纳入编译上下文。**

1. **引用文件**：

   ```
   /// <reference path="path/to/other.d.ts" />
   ```

   这个指令告诉 TypeScript 编译器在编译时包含指定的文件。通常用于引用类型声明文件（`.d.ts` 文件）。
...
2. **引用库**：
   ...
   这个指令用于引用 TypeScript 的内置类型声明库。例如，`node` 类型声明库包含了 Node.js 的全局类型。
...
3. **指定编译选项**：
   ...
   这个指令用于指定编译时使用的库。例如，`es6` 指定了 ES6 的标准库。



##### import/export 

在 `.d.ts` 文件中，使用 `export {} 或者 import` 是一种常见的做法，用于将该文件标记为一个模块。这在 TypeScript 中特别有用，因为默认情况下，一个 `.d.ts` 文件被视为全局声明文件，而不是一个模块。通过添加 `export {}或者 import`，你可以确保该文件被正确地识别为一个模块，从而允许你使用 `import` 和 `export` 语句。

###### 为什么需要 `export {}`

1. **模块识别**：默认情况下，TypeScript 会将一个 `.d.ts` 文件视为全局声明文件。这意味着文件中的声明会直接进入全局作用域。通过添加 `export {}`，你告诉 TypeScript 编译器这是一个模块文件，即使它没有实际的导出内容。
2. **避免全局污染**：将 `.d.ts` 文件标记为模块可以避免意外的全局变量污染，确保你的类型声明只在需要的地方被使用。
...
###### import导致declare module '@/*'无效了

- 因为你在 [globals.d.ts](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 顶部用了 `import`，TypeScript 会把这个文件视为“外部模块（module）”。在模块文件里，顶层的声明不会像“全局环境”那样自动生效，所以你写的 `declare module '@/*' { ... }` 看起来就“失效”了。
- 解决办法有两种（推荐第一种）：把路径别名的 `declare module` 放到一个纯 ambient `.d.ts` 文件（没有任何顶级 `import`/`export`），或者在保留 `import` 的情况下把全局声明放到 `declare global { ... }` 并用 `export {}` 明确模块边界。

![image-20251219141539657](img/前端/项目构建/image-20251219141539657.png)
...
https://mp.weixin.qq.com/s/yRrAQ-8E84SyoLZdU3WOJA
...
- **减轻服务器负担**：文件流不经过应用服务器，节省带宽和计算资源
- **提升上传速度**：用户直接上传到离他最近的 OSS 节点
- **高扩展性**：OSS 专门为海量文件存储设计
- **更安全**：使用临时令牌，不暴露主账号的 AK/SK
...
export default {
  ...
  methods: {
    ...
    async customUpload(options) {
      try {
        // 1. 从后端获取上传策略和凭证
        const policyResponse = await axios.get('/api/oss/policy', {
        ...
        formData.append('key', policy.objectName);        // 存储在OSS的文件路径
        formData.append('policy', policy.policy);         // 上传策略
        formData.append('OSSAccessKeyId', policy.accessKeyId); // 访问密钥
        formData.append('signature', policy.signature);   // 签名
        formData.append('success_action_status', '200');  // 成功状态码
        formData.append('file', options.file);            // 文件本身
        
        // 3. 直接上传到OSS
        const uploadResponse = await axios.post(
          `https://${policy.bucket}.${policy.endpoint}`,
          formData,
          {
        ...
        if (uploadResponse.status === 200) {
          await axios.post('/api/oss/callback', {
...
**Tiptap = ProseMirror 的「富文本 WYSIWYG 精装版」**；
**markdown-it = 纯 Markdown→HTML 的「渲染器」**。

下面把核心差异拆成 6 个维度，一眼看懂：

表格

复制

| 维度            | Tiptap                                                   | markdown-it                                |
| :-------------- | :------------------------------------------------------- | :----------------------------------------- |
| 1. 核心目标     | 富文本**可视化编辑**（WYSIWYG）                          | 把 Markdown **字符串→HTML 字符串**         |
| 2. 技术底座     | ProseMirror 文档模型 + 插件体系                          | 解析器+渲染链，无文档模型                  |
| 3. 交互能力     | 键入、光标、撤销、协作、选区**全套编辑能力**             | **无交互**，只输出静态 HTML                |
| 4. 自定义入口   | 写 Schema / Extension / Command 即可扩展节点、标记、行为 | 写 `markdown-it` 插件改**解析/渲染规则**   |
| 5. 框架集成     | 官方 `@tiptap/vue` `@tiptap/react` 直接当组件用          | 无 UI，需要再包一层编辑器或自己写 textarea |
| 6. 体积（gzip） | 25 kB 核心 + 扩展按需                                    | 10 kB 核心，插件 2-5 kB/个                 |

场景口诀

- 要给用户**「看得见、点得到」的编辑框** → **Tiptap**
- 只要**「MD 字符串→HTML」** → **markdown-it**

一句话记忆

> Tiptap 负责「**编**」；markdown-it 负责「**译**」。

## markdown-it

## tiptap

Ti pTap 采用**分层架构**设计，使用方式取决于你的技术栈：
...
```
npm install @tiptap/pm           # ProseMirror 核心
npm install @tiptap/core         # Ti pTap 核心
npm install @tiptap/starter-kit   # 基础扩展包
```

**这些是纯 JS 包**，不依赖任何框架，可以单独使用：
...
在 TipTap/ProseMirror 的文档模型里，分两大类：

- **node**：块级/结构，比如 paragraph、heading、list、image…
- **mark**：附着在“文本”上的样式/属性，比如 bold、italic、link、highlight…
  - 也就是说，**mark = “给一段文本打的一个标签/样式层”**，不是单独的节点，而是挂在 text node 上的一组属性。

#### **mark（模型层概念）**

列如 [HighlightMark](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 就是自定义的一个 mark 类型，它在文档模型里给某段文字加上 [{ type: highlight, attrs: { id, color, label } }](vscode-file://vscode-app/d:/software/front/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html)
...
将一些最常用的扩展捆绑成了 [`StarterKit`](https://tiptap.zhcndoc.com/editor/extensions/functionality/starterkit)
...
您可以通过传递一个对象来配置 [StarterKit](https://tiptap.zhcndoc.com/editor/extensions/functionality/starterkit) 中包含的所有扩展。要针对特定扩展，配置时请在其前面加上扩展的名称。例如，要将标题级别限制为 1、2 和 3：
...
要排除某些扩展 [StarterKit](https://tiptap.zhcndoc.com/editor/extensions/functionality/starterkit)，可以在配置中将它们设置为 `false`。例如，要禁用 [`Undo/Redo History`](https://tiptap.zhcndoc.com/editor/extensions/functionality/undo-redo) 扩展
...
[StarterKit](https://tiptap.zhcndoc.com/editor/extensions/functionality/starterkit) 不包括所有可用的扩展。要为您的编辑器添加更多功能，请在 `extensions` 数组中列出它们。例如，要添加 [`Strike`](https://tiptap.zhcndoc.com/editor/extensions/marks/strike) 扩展：
...
以下示例将创建一个自定义粗体扩展，不渲染 `<strong>` 标签，而渲染 `<b>` 标签：
...
| 工具            | 作用             | 触发时机               | 检查对象           |
| --------------- | ---------------- | ---------------------- | ------------------ |
| **Husky**       | Git 钩子管理器   | git commit/push 等事件 | 无（它只是触发器） |
| **lint-staged** | 检查代码质量     | pre-commit 阶段        | 你修改的文件       |
| **Commitlint**  | 检查提交信息格式 | commit-msg 阶段        | 你的提交信息       |
...
#    ↓
#    lint-staged 检查 src/App.tsx
#    ↓
#    如果代码有 ESLint 错误 → 提交失败，修复后重试
#    如果通过 → 继续

# 5. Husky 触发 commit-msg hook
#    ↓
#    commitlint 检查提交信息 "添加登录功能"
#    ↓
#    ❌ 格式错误！必须是 "feat(xxx): 描述"
#    ↓
#    提交被阻止！
...
- **husky**操作 git 钩子的工具
  ...
  - `pre-commit`：提交前运行（检查代码）

  - `commit-msg`：提交信息写入后运行（检查提交信息格式）
  - `pre-push`：推送前运行（运行测试）

- **lint-staged**本地暂存代码检查工具

  对 `git add` 过的文件（暂存区文件）运行 linter，而不是检查整个项目
  ...
  ```
  {
    "lint-staged": {
      ...
      "*.{json,md}": [
        "prettier --write"   // JSON/MD 文件只用 Prettier
      ]
    }
  }
  ```
...
ssh-keygen -t rsa -C "120329698@qq.com" -f ~/.ssh/id_rsa_fugle
```
...
![微信图片_20260709153650_63_20](img/前端/项目构建/微信图片_20260709153650_63_20.png)
...
```
# 可选：查看 .gitignore 是否生效
git check-ignore -v packages/vue3-demo/stats.html

# 从索引中移除已被跟踪的文件（只取消跟踪，保留本地文件）
git rm --cached packages/vue3-demo/stats.html
# $lm-config 这个部分，识别成一个变量了，使用通配符来匹配文件
git rm --cached packages/base/*-config/bundle-report.html 

# 将 .gitignore（若你做了修改）和移除记录提交
git add .gitignore
git commit -m "Ignore stats.html"
...
> a分支提交在window上，在mac分支pull a会重新当作修改内容

问题根源，不同操作系统使用不同的行尾符：

- **Windows**: `CRLF`（Carriage Return + Line Feed → `\r\n`）
- **Unix/Linux/Mac**: `LF`（Line Feed → `\n`）
- **经典Mac**: `CR`（Carriage Return → `\r`）
...
| 配置值  | 提交时(Commit) | 检出时(Checkout) | 适用系统            |
| :------ | :------------- | :--------------- | :------------------ |
| `false` | 不转换         | 不转换           | Windows（需要小心） |
| `true`  | **CRLF → LF**  | **LF → CRLF**    | Windows             |
| `input` | **CRLF → LF**  | **不转换**       | **Mac/Linux**       |
...
# 所有开发者都配置
git config --global core.safecrlf warn  # 在 Git 检测到可能造成问题的行尾符转换时，发出警告但不阻止提交。
# 添加文件时 Git 会检查
git add file.txt
# 如果检测到问题，会显示警告：
...
命令面板执行 “Extensions: Show Running Extensions”（查看正在运行的扩展）
...
- cwd：process.cwd() 方法返回 Node.js 进程的当前工作目录。

### path

#### **basename**

path.basename() 方法返回路径的最后一部分，类似于 Unix 的 basename 命令。尾部的目录分隔符会被忽略。
...
它提供了一个安全的沙箱环境来执行 JavaScript 代码。
