...
一个 Fiber节点就对应的是一个组件。对于 classComponent 而言，有 state 是一件很正常的事情，Fiber对象上有一个 memoizedState 用于存放组件的 state。现在看 hooks 所针对的 FunctionComponnet。一个对象都只能有一个 state 属性或者 memoizedState 属性，可是，谁知道开发者们会在 FunctionComponent 里写上多少个 useState，useEffect 等等 ? 所以，react用了链表这种数据结构来存储 FunctionComponent 里面的 hooks。比如： 
...
function batchUpdates(callback) {
  isBatching = true;
  callback(); // 执行所有的 setState
  isBatching = false;
  // 批量执行所有暂存的更新
  performAllUpdates(pendingUpdates);
  pendingUpdates = [];
}
...
在Web开发中，`history.state` 通常用于存储与浏览器历史记录条目相关联的状态信息。这些状态可以在用户导航回之前的页面时被读取。要正确地绑定或管理 `history.state`，你可以通过以下几种方式来实现：

#### 1. 使用 `pushState` 和 `replaceState` 方法

这两个方法允许你添加或修改历史记录条目，并可以指定每个条目的状态对象。
...
当活动历史记录条目改变时（例如用户点击后退按钮），`popstate` 事件会被触发。你可以在这个事件中访问 `event.state` 来获取当前条目的状态对象。
...
**刷新影响：在 `BrowserRouter` 中，刷新页面后 `state` 数据仍然会保留；但若在 `HashRouter` 中刷新，`state` 参数会丢失。**

#### **`History` 模式（v6）**

- **刷新行为**：当用户刷新页面时，浏览器会向服务器发送一个**真实的 HTTP 请求**，请求当前的完整 URL（例如 `https://example.com/users/123`）。服务器需要正确返回对应的 HTML 文件（通常是单页应用的入口文件），否则会出现 404 错误。
- State 行为：
  - **保存位置**：state 保存在浏览器的**会话历史栈**中，关联于当前的历史条目。**它不会随页面刷新而持久化**。
  - **刷新后状态**：页面刷新会**创建一个全新的历史条目**。浏览器会重新加载整个应用，之前通过 `pushState` 或 React Router 的 `navigate` 传递的 `state` **会丢失**，变为 `null` 或 `undefined`。
  - **根本原因**：刷新导致页面重新初始化，JavaScript 执行环境被重建，之前保存在内存中的历史栈信息被清除。虽然浏览器会尝试恢复一部分历史栈（比如回退/前进），但 `pushState` 传递的 `state` 对象**不会自动恢复**到新加载的页面中。
- 开发建议：
  - **不要依赖 `state` 作为唯一数据源**。如果需要持久化，应使用 URL 参数（query string）、Redux/Zustand 等状态管理库（持久化到 localStorage/sessionStorage）、或直接请求后端接口。
  - **典型场景**：临时性页面间数据传递（如列表页到详情页的 ID），刷新后应能通过 URL 中的 ID 重新获取数据。

#### **`Hash` 模式**

- **刷新行为**：刷新页面时，浏览器同样会向服务器发送请求，但请求的是 `#` 之前的路径（例如 `https://example.com/`）。服务器无需针对不同路由配置，只需返回入口 HTML 文件即可。`#` 之后的部分（`#/users/123`）由前端 JavaScript 在页面加载后解析。
- State 行为：
  - **保存位置**：Hash 模式**本身没有原生 state 机制**。React Router 的 `HashRouter` 内部将状态（`state`）编码到了 URL 的**查询字符串**或**路径**中。
  - **刷新后状态**：由于状态被编码到了 URL 中，**刷新后状态不会丢失**。页面重新加载后，React Router 会从哈希值中解析出路径和查询参数，从而完全恢复页面状态。这是 Hash 模式的一个隐蔽优势。
  - **根本原因**：状态被显式地存储在 URL 中，URL 是持久化的，因此刷新后状态自然恢复。
- 开发建议：
  - **适用于**：无法或不想配置服务端路由的静态站点、Electron 应用、部分老旧浏览器兼容场景。
  - **注意事项**：状态被编码到 URL 中可能导致 URL 变得冗长且不美观。同时，`HashRouter` 不支持 `location.key` 和 `action` 等一些高级特性。
