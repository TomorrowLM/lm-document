...
| 微信开放平台网站扫码登录 | PC 页面展示微信登录二维码，用户使用微信扫码登录 | `code`、`state` |
...
`key` 是前后端约定的第三方登录来源配置标识。前端可使用它查询对应配置：
...
| 参数 | 来源 | 作用 |
| --- | --- | --- |
| `key` | 登录回调地址，由前后端约定 | 定位一套第三方登录配置 |
| `mapSource` | 后端配置返回 | 标识登录来源或来源映射 |
| `mapType` | 后端配置返回 | 标识登录类型或映射类型 |
| `mapId` | 后端配置返回 | 标识具体应用、租户或第三方配置 |
| `code` | 微信授权回调返回 | 一次性授权码，由后端换取微信用户身份信息 |
