...
function inMiniProgram() {
  ...
  | 流程 | 典型场景 | 微信返回 |
  | --- | --- | --- |
  | 公众号 OAuth2.0 网页授权 | 用户在微信内打开 H5，需要识别用户身份 | `code`、`state` |
  | 微信开放平台网站扫码登录 | PC 页面展示二维码，用户使用微信扫码登录 | `code`、`state` |

  `key`、`mapSource`、`mapType`、`mapId` 不是微信标准返回字段，而是业务系统用于定位和执行登录的配置参数。

  ### 公众号 OAuth2.0 网页授权

  #### 使用场景

  适用于用户已经在微信内置浏览器中访问 H5，需要识别微信用户身份的场景，例如：

  - 公众号菜单或文章跳转业务 H5 后自动登录
  - 活动报名、问卷、会员中心等需要识别用户的页面
  - 绑定微信账号与手机号
  - 获取用户的 `openid`，并在满足授权范围时获取昵称、头像等信息
  ...
  用户打开微信内 H5
    -> 跳转微信 OAuth2.0 授权页
    -> 用户同意授权
    -> 微信回调 redirect_uri，并携带 code、state
    -> 后端使用 code 换取 access_token、openid
    -> 业务系统创建或恢复登录状态
  ```
  ...
  | --- | --- |
  | `appid` | 公众号的 AppID |
  | `redirect_uri` | 授权完成后的回调地址，需要 URL 编码，并配置在微信授权域名中 |
  | `response_type` | 固定为 `code` |
  | `scope` | 授权范围，例如 `snsapi_base` 或 `snsapi_userinfo` |
  | `state` | 业务侧状态值，用于关联请求和回调，建议使用随机值并校验，防止 CSRF |

  回调地址示例：

  ```text
  https://example.com/login?code=CODE&state=STATE
  ```

  后端拿到 `code` 后，向微信换取授权信息：

  ```http
  GET https://api.weixin.qq.com/sns/oauth2/access_token?appid=APPID&secret=APPSECRET&code=CODE&grant_type=authorization_code
  ```

  如需获取用户信息，再使用返回的 `access_token` 和 `openid` 请求：

  ```http
  GET https://api.weixin.qq.com/sns/userinfo?access_token=ACCESS_TOKEN&openid=OPENID&lang=zh_CN
  ```

  注意：`code` 是一次性的短时授权码，`AppSecret` 只能由后端保存和使用，不能暴露给前端。

  ### 微信开放平台网站扫码登录

  #### 使用场景

  适用于 PC 页面展示微信登录二维码，用户使用手机微信扫码确认登录的场景。该流程使用微信开放平台网站应用的 `snsapi_login` 作用域，不等同于公众号内 H5 OAuth2.0 授权。
  ...
  用户扫码确认后，微信跳转到 `redirect_uri`，并携带一次性 `code` 和原样返回的 `state`。业务后端再使用 `code` 完成用户识别和登录。

  - `appid`、`scope`、`redirect_uri` 是微信登录所需的核心参数
  - `state` 用于关联请求和回调，不能只依赖固定值
  - `key` 不属于微信协议参数，微信不会根据它判断平台或登录方式

  ### 业务系统登录配置映射

  业务系统可以在回调地址中附带 `key`，例如：

  ```text
  /login?key=wxprodgov&code=CODE
  ```

  `key` 是前后端约定的第三方登录来源配置标识。它对微信本身没有业务意义，只作为业务参数随 `redirect_uri` 回调到业务系统。前端可使用 `key` 查询对应配置：

  ```http
  GET /home/dsb/config/get/obj?name=dsb.login.key.{key}
  ```

  后端根据配置返回 `mapSource`、`mapType` 和 `mapId`，前端再将这些业务字段与微信返回的 `code` 一起提交给组织登录接口：

  ```http
  POST /main/new/login/xnzz
  ...
  | 参数 | 来源 | 作用 |
  | --- | --- | --- |
  | `key` | 登录回调地址，由前后端约定 | 定位一套第三方登录配置，例如 `dsb.login.key.{key}` |
  | `mapSource` | 后端配置返回 | 标识登录来源或来源映射，具体数值由后端定义 |
  | `mapType` | 后端配置返回 | 标识登录类型或映射类型，具体数值由后端定义 |
  | `mapId` | 后端配置返回 | 标识具体应用、租户或第三方配置，具体含义由后端定义 |
  | `code` | 微信授权回调返回 | 一次性授权码，由后端用于换取微信用户身份信息 |

  `key` 不是整个登录系统的必填参数。账号登录、验证码登录或已有默认配置的登录流程可以不携带 `key`；但当前这套“通过 `key` 查询第三方登录配置”的流程需要 `key`，否则前端无法定位对应的 `mapSource`、`mapType` 和 `mapId`。如果后端提供默认配置或其他配置查询规则，也可以设计为不依赖 `key`。
...
在按配置处理第三方登录时，回调地址可以携带一个 `key` 参数，例如：

```text
/login?key=wxprodgov&code=CODE
```

`key` 是前后端约定的登录配置标识，不是微信返回的授权码，也不是用户身份凭证。它对微信本身没有业务意义，微信不会根据 `key` 判断登录平台或登录方式；微信真正处理的是 `appid`、`scope`、`redirect_uri` 和 `state` 等授权参数。`key` 只是作为业务参数放在 `redirect_uri` 的查询字符串中，授权完成后随回调地址带回业务系统。前端通过它查询后端配置：

```text
GET /home/dsb/config/get/obj?name=dsb.login.key.{key}
```

后端根据该配置返回 `mapSource`、`mapType` 和 `mapId`，前端再将它们与微信返回的 `code` 一起提交给组织登录接口：
...
| 参数 | 来源 | 作用 |
| --- | --- | --- |
| `key` | 登录回调地址，由前后端约定 | 定位一套登录配置，例如 `dsb.login.key.{key}` |
| `mapSource` | 后端配置返回 | 标识登录来源或来源映射，具体数值由后端定义 |
| `mapType` | 后端配置返回 | 标识登录类型或映射类型，具体数值由后端定义 |
| `mapId` | 后端配置返回 | 标识具体应用、租户或第三方配置，具体含义由后端定义 |
| `code` | 微信授权回调返回 | 微信一次性授权码，由后端用于换取微信用户身份信息 |

`key` 不是整个登录系统的必填参数。账号登录、验证码登录或其他已有默认配置的登录流程可以不携带 `key`；但当前这套“通过 key 查询登录配置”的第三方微信登录流程需要 `key`，否则前端无法定位对应的 `mapSource`、`mapType` 和 `mapId`。后端也可以设计无需 `key` 的配置接口，但需要前后端另行约定获取配置的规则。
...
```mermaid
flowchart TD
    A[点击支付按钮] --> B{pkg 或 order 存在?}
    B -->|否| C[直接返回]
    B -->|是| D[setPaying true 锁定按钮]
    D --> E{已有 orderNo?}
    E -->|否, 有 pkg| F["调用 purchaseOrder 创建订单"]
    E -->|是| G[使用已有 orderNo]
    F --> G
    G --> H{orderNo 有效?}
    H -->|否| C
    H -->|是| I["调用 payOrder 发起支付"]
    I --> J{支付类型?}
    J -->|对公付款 PayTypeEnum.PUBLIC| K["调用 getBankInfo 获取银行信息"]
    K --> L[setBankInfo 展示银行账户]
    L --> M[Toast: 对公付款信息已生成]
    M --> N["回调 onPaid(orderNo)"]
    J -->|支付宝/微信等| O["openPaymentResult 打开支付页面"]
    O --> P["setPollOrderNo 启动轮询"]
    P --> Q[Toast: 已打开支付页面]
    Q --> R["轮询 getOrderStatus 每2秒"]
    R --> S["状态 === PAID?"]
    S -->|否| R
    S -->|是| T[Toast: 支付成功]
    T --> U["回调 onPaid → onClose"]
    N --> V["finally: setPaying false"]
    U --> V
...
https://pay.weixin.qq.com/doc/v3/merchant/4012791845

**商户还需要绑定小程序appid才能支付**

**微信支付需要授权域名：h5域名，jsapi域名**

微信支付的 H5 支付域名配置支持一级域名

JSAPI 支付的授权目录配置**不支持一级域名**，必须精确到**具体目录层级**

| 环境                | JSAPI    | H5 (mwebUrl) | 推荐方案           | 是否需要 AppID           | 是否需要 openid |
| :------------------ | :------- | :----------- | :----------------- | :----------------------- | :-------------- |
| **微信内置浏览器**  | ✅ 可用   | ❌ 被拦截     | **JSAPI**          | 公众号 AppID             | 是，必传        |
| **外部浏览器**      | ❌ 无 API | ✅ 可用       | **H5 支付**        | 商户号 AppID（下单时传） | 否，不传        |
| **小程序 web-view** | ❌ 被剥离 | ❌ 被拦截     | **小程序原生支付** | 小程序 AppID             | 是，必传        |

### h5支付

WeChat H5 支付的 [mwebUrl](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html) 不能通过 [window.open](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html) 打开，需要用 [location.href](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html) 做当前页跳转。这是微信 H5 支付的规范要求：

- **[window.open](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)** → 新标签页没有正确的 Referer，微信校验失败 → "商家参数格式有误"
- **[location.href](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)** → 当前页跳转，Referer 正确传递，微信校验通过，支付完成后可通过 `redirect_url` 回到页面

|      | 支付宝                                                       | 微信 H5 支付                                                 |
| ---- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 方式 | [window.open](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)（新标签） ✅ | [window.open](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)（新标签） ❌ |
| 原因 | 支付宝不校验 Referer                                         | 微信校验 Referer，新标签页没有，报 **"商家参数格式有误"**    |
| 修复 | 不变                                                         | 改为 [location.href = mwebUrl](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)（当前页跳转） |

### 微信浏览器支付

微信明确禁止在微信内置浏览器中使用 H5 支付，这不是技术限制，而是微信的产品设计规则：

| 环境                        | 可用方式                                                     |
| --------------------------- | ------------------------------------------------------------ |
| 微信内置浏览器              | **仅 JSAPI**（[WeixinJSBridge.invoke](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)） |
| 外部浏览器（Safari/Chrome） | **仅 H5 支付**（[mwebUrl](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)） |

如果在微信内硬走 [location.href = mwebUrl](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)，微信中间页 `checkmweb` 会检测到 UA 含 `MicroMessenger`，直接拦截并提示：

> **"请在微信外打开订单，进行支付"**

这是微信支付的硬性规则，没有变通方式。当前代码的双分支策略（UA 检测 → JSAPI / H5）就是正确的做法。

### 小程序 `<web-view>` 

 微信小程序 `<web-view>` 中 H5 支付也会被拦截，因为 UA 仍然包含 `MicroMessenger`，`checkmweb` 一样会报 "请在微信外打开订单"。

```mermaid
flowchart LR
    A["小程序 web-view"] --> B{"checkmweb 检测 UA"}
    B -->|"含 MicroMessenger ❌"| C["请在微信外打开订单"]
    B -->|"不含 MicroMessenger ✅"| D["正常跳转微信支付页"]
```

H5 无法直接完成支付，需要通过 `postMessage` 通知小程序原生层去调 `wx.requestPayment`：

```
H5 → wx.miniProgram.postMessage({ action: 'pay', orderNo: 'xxx' })
    → 小程序原生层收到 → wx.requestPayment(...)
```

当前代码如果要在小程序中兜底，可以加一个 `isMiniProgram` 判断，走 H5 的 [mwebUrl](vscode-file://vscode-app/Applications/Visual Studio Code.app/Contents/Resources/app/out/vs/code/electron-browser/workbench/workbench.html)——虽然 `checkmweb` 大概率也会拦截，但至少能给出微信支付的"请在浏览器打开"提示页，用户可以从那里复制链接或手动操作。**彻底解决还是需要小程序原生支付。**



## 小程序 WebView H5 图片/文档预览与下载

### 结论：预览和下载是两条完全独立的路径

| 目标               | 能否在 WebView H5 内直接实现 | 条件                 |
| ------------------ | ---------------------------- | -------------------- |
| 图片预览           | ✅ 可以（`<img>` 标签）       | 零配置               |
| 图片下载保存       | ❌ 不行                       | 必须跳到小程序原生页 |
| 文档预览（PDF 等） | ⚠️ 仅当文件域名已配业务域名   | 第三方域名基本做不到 |
| 文档下载保存       | ❌ 不行                       | 必须跳到小程序原生页 |
...
| 文件类型 | 实现方式                     | 前提条件                              | 效果                           |
| -------- | ---------------------------- | ------------------------------------- | ------------------------------ |
| 图片     | H5 内 `<img src="URL">`      | **零配置**，不经过任何域名白名单      | 正常显示，iOS 长按保存兼容性差 |
| 图片     | 小程序原生 `wx.previewImage` | **downloadFile 合法域名** 配好        | 完美预览 + 长按保存            |
| 图片     | `<a>` 跳转图片 URL           | **业务域名** 配好（第三方基本不可能） | ❌ 第三方域名直接报错拦截       |
| PDF/文档 | `window.location.href` 跳转  | **业务域名** 配好                     | 同域 webview 内预览，不能保存  |
| PDF/文档 | 小程序原生 `wx.openDocument` | **downloadFile 合法域名** 配好        | 打开系统预览，可分享收藏       |
| PDF/文档 | `<a>` 跳转文档 URL           | **业务域名** 配好（第三方基本不可能） | ❌ 第三方域名直接报错拦截       |
...
| 方式                              | 图片 | 文档 | 条件                                |
| --------------------------------- | ---- | ---- | ----------------------------------- |
| `<a download>`                    | ❌    | ❌    | 小程序 WebView 内完全失效           |
| `wx.downloadFile` + `wx.saveFile` | ✅    | ✅    | **downloadFile 合法域名** 配好      |
| 后端代理中转                      | ✅    | ✅    | 仅需配**自有域名**到 downloadFile   |
| 长按保存（图片）                  | ⚠️    | —    | 仅 `wx.previewImage` 原生预览时可用 |
...
```
是否涉及页面跳转（<a>/window.open/window.location）
  ├─ 是 → 校验【业务域名】→ 第三方域名无校验文件 → ❌ 拦截
  └─ 否 → 不校验业务域名

是否调用小程序原生 API（downloadFile/previewImage/openDocument）
  ├─ 是 → 校验【downloadFile 合法域名】→ 可配自有域名 → ✅ 可行
  └─ 否 → 不校验下载域名
```
...
```
  ...
  仅预览              下载/保存
    ...
    │   可预览           wx.downloadFile
    │                    ↓
    └─ 第三方域名：      wx.openDocument / saveFile
        走原生预览       （前提：downloadFile 合法域名已配）
```
...
| ---------------------------- | --------------------------------------------- |
| H5 里看图片                  | **不需要任何配置**                            |
| H5 里看第三方 PDF            | **不可能**，必须跳原生页                      |
| 下载保存任何文件             | 配 **downloadFile 合法域名** + 跳小程序原生页 |
| 第三方域名完全不可控         | 上**后端代理**，只配自有域名                  |
| *（内容由AI生成，仅供参考）* |                                               |
