...
- 云服务前端部署  网络问题卡死 。
  ...
  - 显示详细日志以定位问题 npm install --verbose

    | 日志级别    | 作用                                            | 适用场景                                      |
    | :---------- | :---------------------------------------------- | :-------------------------------------------- |
    | **silent**  | **完全不输出任何日志**（包括错误）              | 自动化脚本、CI/CD流水线（不想让日志污染输出） |
    | **error**   | 仅输出错误信息                                  | 生产环境，只关注致命错误                      |
    | **warn**    | 输出错误和警告（默认级别）                      | 日常开发                                      |
    | **notice**  | 输出错误、警告和重要通知                        | 需要更多信息时                                |
    | **http**    | 输出 HTTP 请求相关的日志                        | 调试网络请求问题                              |
    | **timing**  | 输出详细的耗时信息                              | 性能分析                                      |
    | **info**    | 输出详细信息（如你之前日志中的 `npm info run`） | 查看安装过程的详细信息                        |
    | **verbose** | 输出最详细的调试信息                            | 排查复杂问题                                  |

    - 原因：npm install下载的依赖中会执行 `postinstall` 脚本（如编译、下载二进制文件）

      https://github.com/zloirock/core-js#postinstall-message

      **取消脚本执行：** 使用 `npm install --ignore-scripts`，防止第三方包执行 `preinstall`、`postinstall` 等脚本

      **适用场景：**

      - 遇到像你之前遇到的 `puppeteer` 下载 Chrome 卡死的问题
      - 在 CI/CD 环境中，不需要执行这些脚本
      - 安全考虑，防止恶意包执行不明脚本
      - 安装速度优化（跳过不必要的步骤）

    - 解决：npm install --legacy-peer-deps --ignore-scripts
...
> 九小场所是指建筑面积300平方米以下的小商场、额定就餐人数100人以下的小饭店、床位数50张以下的小旅馆，以及小型的公共娱乐、休闲健身、医疗、教学、生产加工、易燃易爆危险品销售储存等场所，具体涵盖小型学校幼儿园、小医疗机构、小商店、小餐饮、小旅店、小歌舞娱乐、小网吧、小美容洗浴、小生产加工企业等类型

- 企业

  - 企业类别：一般企业 园区企业 承租企业
  - 消防类型：消防重点单位 一般单位 九小场所
...
- 基础信息
  ...
  ![image-20260130165529513](img/公司/fugle/image-20260130165529513.png)
  ...
  - 证件资质
    ...
    - 人员
      ...
      <img src="img/公司/fugle/image-20260130165330943.png" alt="image-20260130165330943" style="zoom: 25%;" />

    - 企业

      <img src="img/公司/fugle/image-20260130165356584.png" alt="image-20260130165356584" style="zoom: 25%;" />
...
**从镇街中拿到街道下的企业录入信息，用户填写采集信息选择企业后提交即可注册一起安**
